...
mod tests {
    ...
    use rust_ai_ide_protocol::{commands::*, errors::ProtocolError};
    use rust_ai_ide_common::test_utils::{setup::*, generators::*};
    ...
    async fn test_file_system_handlers_integration() {
        ...
        let files =
            vec![rust_ai_ide_protocol::commands::fs::FileInfo {
        ...
        assert!(files.iter().any(|f| f.name == "test.rs"), "Should find the created file");

        // Test file checksum handler
        let checksum_result = generate_checksum(&test_file);
        assert!(checksum_result.is_ok(), "Checksum generation should succeed");
        assert!(!checksum_result.unwrap().is_empty(), "Checksum should not be empty");
        ...
        assert_eq!(watch_request.path, test_file.to_string_lossy(), "Watch request should contain correct path");
    ...
    async fn test_git_handlers_integration() {
        ...
        assert_eq!(status_request.directory, "/tmp", "Status request should have correct directory");
        ...
        assert!(!add_request.paths.is_empty(), "Add request should have specified paths");
        ...
        assert_eq!(commit_request.message, "Test commit", "Commit request should have correct message");
        assert_eq!(commit_request.author_name, Some("Test Author".to_string()), "Commit should have author name");
    }
    ...
    async fn test_protocol_error_serialization() {
        ...
        assert!(matches!(deserialized, ProtocolError::Validation(_)), "Error type should be preserved during serialization");
        ...
        assert!(matches!(file_deserialized, ProtocolError::FileSystem(_)), "File system error should be preserved");
        ...
        assert!(matches!(processing_deserialized, ProtocolError::Processing(_)), "Processing error should be preserved");
    }
    ...
    async fn test_rate_limiting_integration() {
        ...
        assert!(elapsed < Duration::from_millis(500), "Rate limiting test should complete quickly");
    }
    ...
    async fn test_event_bus_integration() {
        ...
        // Send test event
        event_bus.emit("test_channel", serde_json::json!({"test": "data"})).await;
    ...
    async fn test_handler_validation_integration() {
        ...
        assert!(validate_secure_path(valid_path, false).is_ok(), "Valid path should pass");

        let invalid_path = "/tmp/test/../../../etc/passwd";
        assert!(validate_secure_path(invalid_path, false).is_err(), "Path traversal should fail");

        let absolute_when_denied = "/absolute/path/forbidden";
        assert!(validate_secure_path(absolute_when_denied, false).is_err(), "Absolute paths should be denied when not allowed");

        // Test string input validation
        let valid_string = "valid_input_123";
        assert!(validate_string_input(valid_string, 50, false).is_ok(), "Valid string should pass");

        let too_long_string = &"a".repeat(100);
        assert!(validate_string_input(too_long_string, 50, false).is_err(), "Too long string should fail");

        let forbidden_chars = "invalid;input()";
        assert!(validate_string_input(forbidden_chars, 50, true).is_ok(), "Special chars should pass when allowed");
        assert!(validate_string_input(forbidden_chars, 50, false).is_err(), "Special chars should fail when forbidden");
    }
    ...
    async fn test_project_lifecycle_integration() {
        ...
        if let Ok(_) = valid_project {
            ...
            println!("Project validation successful for: {}", project_path.display());
        }
    ...
    fn generate_checksum(file_path: &str) -> Result<String, ProtocolError> {
        ...
        let content = fs::read(file_path)
            .map_err(|e| ProtocolError::FileSystem(format!("Read error: {}", e)))?;
