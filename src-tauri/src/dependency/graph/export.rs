...
pub fn export_graph(graph: &DependencyGraph, format: ExportFormat) -> Result<Vec<u8>> {
    match format {
    ...
    for edge in graph.graph().edge_references() {
        if let (Some(source), Some(target)) = (
            ...
            let style = match edge.weight().dep_type {
                super::DependencyType::Dev => "style=dashed",
                super::DependencyType::Build => "style=dotted",
    ...
    for edge in graph.graph().edge_references() {
        if let (Some(source), Some(target)) = (
            ...
            let edge_type = match edge.weight().dep_type {
                super::DependencyType::Normal => "normal",
                super::DependencyType::Dev => "dev",
