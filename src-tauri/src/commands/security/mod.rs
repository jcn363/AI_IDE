...
use crate::security::vulnerability_scanner::{VulnerabilityScanner, VulnerabilityReport as VulnerabilityReportSimple};

/// Security and vulnerability scanning commands using existing modules
#[tauri::command]
pub async fn scan_for_vulnerabilities(project_path: String) -> Result<Vec<crate::security::rustsec_integration::VulnerabilityReport>, String> {
    ...
    scanner.scan_lockfile(&lockfile_path)
...
pub async fn load_license_policy(policy_path: String) -> Result<license::LicensePolicy, String> {
    license::LicensePolicy::from_file(Path::new(&policy_path))
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn save_license_policy(policy_path: String, policy: license::LicensePolicy) -> Result<(), String> {
    policy.save_to_file(Path::new(&policy_path))
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn check_license_against_policy(license: String, policy: license::LicensePolicy) -> Result<license::LicenseCompliance, String> {
