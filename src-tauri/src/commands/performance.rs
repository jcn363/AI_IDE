            ...
            Some(monitor) => {
                match monitor.collect_metrics().await {
                    Ok(metrics) => {
                        ...
                        let _ = event_bus.emit("performance:system_metrics", serde_json::json!({
...
pub async fn get_performance_history(
    ...
    execute_command!("get_performance_history", &Default::default(), async move || {
...
pub async fn detect_memory_leaks(
    ...
    execute_command!("detect_memory_leaks", &Default::default(), async move || {
    ...
    execute_command!("optimize_memory", &Default::default(), async move || {
        ...
        match optimizer_guard.as_mut() {
            Some(optimizer) => {
                ...
                let total_freed = optimizations.iter()
                    .map(|o| o.memory_freed_bytes).sum::<usize>();
...
pub async fn get_optimization_stats(
    ...
    execute_command!("get_optimization_stats", &Default::default(), async move || {
...
pub async fn get_process_metrics(
    ...
    execute_command!("get_process_metrics", &Default::default(), async move || {
        let monitor_guard = state.performance_monitor.lock().await;
        match monitor_guard.as_ref() {
            Some(monitor) => {
...
pub async fn get_resource_alerts(
    ...
    execute_command!("get_resource_alerts", &Default::default(), async move || {
        let monitor_guard = state.performance_monitor.lock().await;
        match monitor_guard.as_ref() {
            Some(monitor) => {
...
#[tauri::command]
pub async fn get_parallel_stats(
    state: State<'_, AppState>
) -> Result<String, String> {
...
#[tauri::command]
pub async fn get_cross_platform_memory(
    state: State<'_, AppState>
) -> Result<String, String> {
    Ok(serde_json::json!({
        "status": "info",
...
pub async fn initialize_performance_monitoring(
    ...
    execute_command!("initialize_performance_monitoring", &Default::default(), async move || {
