...
use rust_ai_ide_core::validation::{validate_secure_path, validate_rust_project, validate_file_size};
...
pub async fn build_project(project_path: String) -> Result<String, String> {
    ...
    if !validation_result.is_valid {
        return Err(format!("Project validation failed: {:?}", validation_result.errors));
    }

    log::info!("Building project at: {}", project_path);

    // Execute cargo build using unified command execution
    let result = execute_command("cargo", &["build", "--message-format=json"])
    ...
    if result.success {
        ...
        let error_msg = result.stderr.unwrap_or_else(|| "Build failed without error output".to_string());
...
pub async fn run_project(project_path: String) -> Result<String, String> {
    ...
    if !validation_result.is_valid {
        return Err(ContextualError::new(
            IDEError::ValidationError(format!("Project validation failed: {:?}", validation_result.errors)),
    ...
    build_project(project_path.clone()).await
        .map_err(|e| ContextualError::new(
            e,
    ...
    let bin_name = std::fs::read_dir(&target_dir)
        ...
        .next()
        .ok_or_else(|| ContextualError::new(
...
pub async fn test_project(project_path: String) -> Result<String, String> {
    ...
    let result = execute_command("cargo", &["test", "--", "--nocapture"])
        .map_err(|e| ContextualError::new(
...
pub async fn doc_generate(project_path: String) -> Result<String, String> {
    ...
    let result = execute_command("cargo", &["doc", "--no-deps"])
        .map_err(|e| ContextualError::new(
            IDEError::CommandExecutionError(format!("Failed to execute cargo doc: {}", e)),
            format!("Documentation generation failed for project: {}", project_path)
        ))?;
    ...
    if result.success {
        ...
        let error_msg = result.stderr.unwrap_or_else(|| "Documentation generation failed without error output".to_string());
...
pub async fn doc_read_file(path: String) -> Result<String, String> {
    ...
    validate_file_extension(path_obj, &["html", "htm"])
        .map_err(|e| ContextualError::new(
    ...
    validate_file_size(fs::metadata(&path)
        .map_err(|e| ContextualError::new(
