    ...
    execute_command!(
        ...
        async move || {
            ...
            let rate_limit_result = rate_limiter
                .check_rate_limit(&user, EndpointType::WebauthnRegistration, None)
                .await;
...
pub async fn webauthn_start_authentication(
    ...
    execute_command!(
        ...
        async move || {
            ...
            // Apply rate limiting
            let rate_limit_result = rate_limiter
                .check_rate_limit(&user_context, EndpointType::WebauthnAuthentication, None)
                .await;
            ...
            acquire_service_and_execute!(state.webauthn_service, WebAuthnService, {
                let challenge_response = service
                    .start_authentication(&sanitized_input.user_id)
                    .await?;
...
pub async fn webauthn_delete_credential(
    ...
    execute_command!(
        ...
        async move || {
            ...
            acquire_service_and_execute!(state.webauthn_service, WebAuthnService, {
                service
                    .delete_credential(&user, &sanitized_input.credential_id)
                    .await?;
...
#[tauri::command]
pub async fn webauthn_get_status(state: State<'_, WebAuthnService>) -> Result<String, String> {
    execute_command!(
        "webauthn_get_status",
        &WEBAUTHN_COMMAND_CONFIG,
        async move || {
