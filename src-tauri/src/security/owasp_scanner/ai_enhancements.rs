...
impl AIEnhancedAnalyzer {
    ...
    fn calculate_correlation_strength(&self, vuln_a: &OWASPVulnerability, vuln_b: &OWASPVulnerability) -> f32 {
        let category_similarity = if vuln_a.owasp_category.to_string() == vuln_b.owasp_category.to_string() {
            1.0
        } else {
            0.3
        };
        let severity_similarity =
            if vuln_a.security_issue.severity.to_string() == vuln_b.security_issue.severity.to_string() {
                1.0
            } else {
                0.5
            };
