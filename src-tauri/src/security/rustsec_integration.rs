...
impl RustsecScanner {
    ...
    pub fn new() -> Result<Self> {
        let db = Database::fetch()
            .context("Failed to fetch RustSec advisory database")?;
    ...
    pub fn scan_lockfile(&self, lockfile_path: &Path) -> Result<Vec<VulnerabilityReport>> {
        let lockfile = rustsec::Lockfile::load(lockfile_path)
            .context("Failed to load Cargo.lock")?;
        ...
        for vulnerability in self.db.vulnerabilities(&lockfile) {
            ...
            reports.push(VulnerabilityReport {
                ...
                severity: format!("{:?}", advisory.cvss.as_ref().map(|c| c.severity()).unwrap_or(rustsec::Severity::Medium)),
                cvss: advisory.cvss.as_ref().map(|c| c.score().to_f32().unwrap()),
                patched_versions: vuln.versions.patched().iter()
                    .map(|v| v.to_string())
                    .collect(),
                unaffected_versions: vuln.versions.unaffected().iter()
                    .map(|v| v.to_string())
                    .collect(),
            });
        }
    ...
    pub fn update_database(&mut self) -> Result<()> {
        self.db = Database::fetch()
            .context("Failed to update RustSec advisory database")?;
        Ok(())
    }
