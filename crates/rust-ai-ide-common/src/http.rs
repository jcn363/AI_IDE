...
pub trait RequestInterceptor: Send + Sync {
    async fn intercept(&self, request: &mut HttpRequest<serde_json::Value>) -> Result<(), HttpError>;
}

/// Response interceptor trait
#[async_trait::async_trait]
pub trait ResponseInterceptor: Send + Sync {
    async fn intercept(&self, response: &mut HttpResponse<serde_json::Value>) -> Result<(), HttpError>;
}
...
impl RequestInterceptor for RequestInterceptorEnum {
    async fn intercept(&self, request: &mut HttpRequest<serde_json::Value>) -> Result<(), HttpError> {
        match self {
            RequestInterceptorEnum::Logging(interceptor) => RequestInterceptor::intercept(interceptor, request).await,
        }
    }
}
...
impl HttpClient {
    ...
    async fn execute_with_retry(&self, request: HttpRequest<serde_json::Value>) -> Result<Response, HttpError> {
        let call_id = self
            .call_count
    ...
    async fn execute_single(&self, request: &HttpRequest<serde_json::Value>) -> Result<Response, HttpError> {
        ...
        let final_url = if let Some(base_url) = &self.config.base_url {
            ...
            reqwest::Url::parse(&request.url).map_err(|e| HttpError::Protocol(format!("URL parse error: {}", e)))?
        };
    ...
    pub async fn post<B, T>(&self, url: impl Into<String>, body: B) -> Result<HttpResponse<T>, HttpError>
    where
        B: Serialize + Send + Sync,
        T: DeserializeOwned + Serialize,
    {
