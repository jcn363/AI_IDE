...
fn safety_guard_benchmark(c: &mut Criterion) {
    ...
    for (size_name, count) in &change_counts {
        ...
        group.bench_with_input(
            ...
            |b, changes| {
                b.iter(|| {
                    let risks: Vec<_> = changes
                        .iter()
                        .map(|c| guard.assess_risk(c))
                        .collect();
                    black_box(risks);
                });
            },
    ...
    for &threshold in &risk_thresholds {
        group.bench_with_input(
            ...
            |b, &threshold| {
                ...
                b.iter(|| {
                    let risky_changes: Vec<_> = changes
                        .iter()
                        .filter(|c| guard.is_risky(c))
                        .count();
    ...
    group.bench_function("complex_validation_scenario", |b| {
        ...
        b.iter(|| {
            ...
            let safe_changes: Vec<_> = changes
                .iter()
                .filter(|c| !guard.is_high_risk(c))
                .collect();
