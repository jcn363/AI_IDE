    ...
    pub fn new() -> Self {
        ...
        templates.insert("typescript".to_string(), LanguageTemplates {
            ...
            interface_template:       "export interface {{name}} {{#if extends}}extends {{extends}} {{/if}}{\n    \
                                       {{#each fields}}\n    {{name}}: {{type}};\n    {{/each}}\n}"
                .to_string(),
            type_definition_template: "export type {{name}} = {{#if union}}{{union}}{{else}}{{#if object}}{ {{#each \
                                       fields}}{{name}}: {{type}};{{/each}} }{{else}}{{primitive}}{{/if}}{{/if}};"
                .to_string(),
        ...
        templates.insert("python".to_string(), LanguageTemplates {
            function_template:
                "def {{name}}({{parameters}}) {{#if return_type}}-> {{return_type}}{{/if}}:\n    {{#each body}}\n    \
                 {{this}}\n    {{/each}}"
                    .to_string(),
            class_template:           "class {{name}}{{#if extends}}({{extends}}){{/if}}:\n    def \
                                       __init__(self{{#each fields}}, {{name}}: {{type}}{{/each}}):\n        {{#each \
                                       fields}}\n        self.{{name}} = {{name}}\n        {{/each}}\n\n    {{#each \
                                       methods}}\n    {{this}}\n    {{/each}}"
                .to_string(),
            interface_template:       "from abc import ABC, abstractmethod\n\nclass {{name}}(ABC):\n    {{#each \
                                       methods}}\n    @abstractmethod\n    {{this}}\n    {{/each}}"
                .to_string(),
            type_definition_template: "# Type hint for {{name}}\n{{name}}: {{#if union}}{{union}}{{else}}{{#if \
                                       object}}Dict[str, Any]{{else}}{{primitive}}{{/if}}{{/if}}"
                .to_string(),
            async_function_template:  "async def {{name}}({{parameters}}) {{#if return_type}}-> \
                                       {{return_type}}{{/if}}:\n    {{#each body}}\n    {{this}}\n    {{/each}}"
                .to_string(),
        });
        ...
        templates.insert("go".to_string(), LanguageTemplates {
            ...
            type_definition_template: "// Type definition\ntype {{name}} {{#if \
                                       union}}interface{}{{else}}struct{{/if}} {\n    {{#if union}}\n    // Union \
                                       type represented as interface\n    {{#else}}\n    // Struct fields here\n    \
                                       {{/if}}\n}"
                .to_string(),
            async_function_template:  "// Go uses goroutines for concurrency\nfunc {{name}}({{parameters}}) {{#if \
        ...
        templates.insert("java".to_string(), LanguageTemplates {
            function_template:        "public {{#if static}}static {{/if}}{{return_type|void}} \
                                       {{name}}({{parameters}}) {\n    {{#each body}}\n    {{this}}\n    {{/each}}\n}"
                .to_string(),
            class_template:           "public class {{name}} {{#if extends}}extends {{extends}} {{/if}}{{#if \
                                       implements}}implements {{implements}} {{/if}}{\n    {{#each fields}}\n    \
            ...
            type_definition_template: "// Java type definition - use classes or interfaces\n// {{name}} would be \
                                       implemented as a class or interface"
                .to_string(),
            async_function_template:  "// Java async - would use CompletableFuture or similar\npublic \
                                       CompletableFuture<{{return_type|Void}}> {{name}}({{parameters}}) {\n    return \
        ...
        templates.insert("javascript".to_string(), LanguageTemplates {
            ...
            interface_template:       "// JavaScript doesn't have interfaces - using JSDoc\n/**\n * @interface \
                                       {{name}}\n {{#each methods}}\n * @method {{name}}\n {{/each}}\n */\nfunction \
                                       {{name}}() {}\n\n// Implementation would be a regular object"
                .to_string(),
            type_definition_template: "// JSDoc type definition\n/**\n * @typedef {{{#if \
                                       union}}{{union}}{{else}}{{#if \
                                       object}}Object{{else}}{{primitive}}{{/if}}{{/if}}} {{name}}\n */"
                .to_string(),
            async_function_template:  "async function {{name}}({{parameters}}) {\n    {{#each body}}\n    {{#if \
                                       ../is_await}}await {{/if}}{{this}}\n    {{/each}}\n}"
                .to_string(),
        });
        ...
        templates.insert("cpp".to_string(), LanguageTemplates {
            function_template:        "{{return_type|void}} {{name}}({{parameters}}) {\n    {{#each body}}\n    \
                ...
                .to_string(),
            class_template:
                "class {{name}} {{#if extends}}: public {{extends}} {{/if}}{\n{{#each \
                 visibility_sections}}\n{{visibility}}:\n    {{#each fields}}\n    {{type}} {{name}};\n    \
                 {{/each}}\n\n    {{#each methods}}\n    {{#if virtual}}virtual {{/if}}{{return_type|void}} \
                 {{name}}({{parameters}}){{#if const}} const{{/if}} {{#if override}} override{{/if}} {\n        \
                 {{#each body}}\n        {{this}}\n        {{/each}}\n    }\n    {{/each}}\n{{/each}}\n};"
                    .to_string(),
            interface_template:       "class {{name}} {\npublic:\n    {{#each methods}}\n    virtual \
                                       {{return_type|void}} {{name}}({{parameters}}) = 0;\n    {{/each}}\n    virtual \
                                       ~{{name}}() = default;\n};"
                .to_string(),
            type_definition_template: "// C++ type definition\n// Complex types would use templates or \
                                       variants\nusing {{name}} = {{#if \
                                       union}}std::variant<{{union_types}}>{{else}}{{#if \
                                       primitive}}{{primitive}}{{else}}auto{{/if}}{{/if}};"
                .to_string(),
            async_function_template:  "std::future<{{return_type|void}}> {{name}}({{parameters}}) {\n    return \
                                       std::async(std::launch::async, []() {\n        {{#each body}}\n        \
                                       {{this}}\n        {{/each}}\n        return {{#if \
                                       return_type}}result{{else}}{} {{/if}};\n    });\n}"
                .to_string(),
        });
