...
use crate::types::{
    ModelId, ModelInfo, ModelMetrics, RequestContext, ModelRecommendation,
    ModelTask, Complexity, RequestPriority, PerformanceThresholds, ModelSwitchingConfig,
};
...
impl ModelSelectionEngine {
    pub async fn select_model(
        ...
        for model_info in models.iter() {
            ...
            if let Some(last_switch) = cooldowns.get(&model_info.id) {
                if last_switch.elapsed() < std::time::Duration::from_secs(
            ...
            let score = self.calculate_selection_score(
                &model_info.id,
                context,
                performance_tracker,
                load_predictor,
    ...
    fn is_model_compatible(&self, capability: &crate::types::ModelCapability, request: &RequestContext) -> bool {
        ...
        match request.task_type {
            ModelTask::Analysis | ModelTask::Refactoring if !capability.supported_languages.contains(&"rust".to_string()) => {
                return false;
            }
    ...
    async fn calculate_selection_score(
        ...
        let resource_score = if system_resources.available_memory_mb > 2048.0 { 1.0 } else { 0.8 };
...
impl PerformanceBasedModelSelector {
    ...
    pub async fn select_model(&self, context: &RequestContext) -> Result<ModelRecommendation> {
        self.selection_engine.select_model(
            context,
    ...
    pub async fn update_model_metrics(&self, model_id: ModelId, metrics: ModelMetrics) -> Result<()> {
        self.performance_tracker.update_metrics(model_id, metrics).await?;

        // Also update load predictor
        let load_factor = metrics.cpu_usage_percent / 100.0; // Convert to 0-1 scale
        self.load_predictor.record_load(model_id, load_factor).await?;
...
mod tests {
    ...
    async fn test_model_selection() {
        ...
        selector.update_model_metrics(test_model_id, metrics).await.unwrap();
