...
impl AsyncDatabase {
    ...
    pub async fn new<T: AsRef<Path>>(db_path: T) -> AIResult<Self> {
        ...
        let conn = task::spawn_blocking(move || Connection::open(db_path))
            .await
    ...
    pub async fn execute_batch(&self, sql: &str) -> AIResult<()> {
        let sql = sql.to_string();
        task::spawn_blocking(move || self.conn.execute_batch(&sql))
            .await
            .map_err(|e| AIServiceError::DatabaseError(format!("Async spawn error: {}", e)))?
...
impl PreparedStatement {
    ...
    where
        ...
        T: Send + 'static,
    {
        task::spawn_blocking(move || match self.stmt.query_row(params, mapper) {
            Ok(result) => Ok(Some(result)),
            Err(rusqlite::Error::QueryReturnedNoRows) => Ok(None),
            Err(e) => Err(e),
        })
