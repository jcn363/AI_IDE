...
fn test_security_analyzer_detects_command_injection() {
    ...
    let has_command_injection = findings.iter().any(|finding| 
        finding.issue_type == SecurityIssueType::CommandInjection
    );
    
    assert!(has_command_injection, "Expected to find command injection issue");
}
...
fn test_security_analyzer_with_safe_code() {
    ...
    assert!(findings.is_empty(), "Expected no security issues in safe code");
}
...
fn test_security_analyzer_disabled() {
    ...
    assert!(!analyzer.is_enabled(&prefs), "Security analyzer should be disabled");
}
...
fn test_security_analyzer_enabled() {
    ...
    assert!(analyzer.is_enabled(&prefs), "Security analyzer should be enabled");
}

#[test]
fn test_security_analyzer_integration() {
    // This test verifies that the security analyzer works with the analysis registry
    use crate::analysis::{AnalysisRegistry, AnalysisPreferences};
    ...
    // Check that we found the command injection issue
    let has_security_issue = findings.iter()
        .any(|finding| finding.category == AnalysisCategory::Security);
        
    assert!(has_security_issue, "Expected to find security issues in analysis results");
}
