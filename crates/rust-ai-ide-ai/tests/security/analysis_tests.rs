...
use rust_ai_ide_ai::{
    analysis::{
        security::{InsecureCryptoDetector, HardcodedSecretsDetector, SqlInjectionDetector},
...
fn test_hardcoded_secrets_detection() {
    ...
    assert!(
        !result.findings.iter().any(|f| f.message.contains("MAX_RETRIES") || f.message.contains("TIMEOUT")),
...
fn test_sql_injection_detection() {
    ...
    assert!(
        !result.findings.iter().any(|f| f.message.contains("get_user_safe") || f.message.contains("get_user_safe_builder")),
...
fn test_security_analyzers_together() {
    ...
    assert!(
        result.findings.iter().any(|f| f.analysis_type == AnalysisType::InsecureCrypto),
        "Expected to find insecure crypto usage"
    );
    
    assert!(
        result.findings.iter().any(|f| f.analysis_type == AnalysisType::HardcodedSecret),
        "Expected to find hardcoded secrets"
    );
    
    assert!(
        result.findings.iter().any(|f| f.analysis_type == AnalysisType::SqlInjection),
...
fn test_security_finding_severity() {
    ...
    // Check severities
    let sql_injection = result.findings.iter().find(|f| f.analysis_type == AnalysisType::SqlInversion);
    assert_eq!(sql_injection.unwrap().severity, Severity::Critical, "SQL injection should be Critical");
    
    let insecure_crypto = result.findings.iter().find(|f| f.analysis_type == AnalysisType::InsecureCrypto);
    assert_eq!(insecure_crypto.unwrap().severity, Severity::High, "Insecure crypto should be High");
    
    let hardcoded_secret = result.findings.iter().find(|f| f.analysis_type == AnalysisType::HardcodedSecret);
    assert_eq!(hardcoded_secret.unwrap().severity, Severity::Medium, "Test API key should be Medium");
}
