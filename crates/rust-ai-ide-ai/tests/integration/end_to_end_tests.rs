...
use rust_ai_ide_ai::{
    analysis::{
        ...
        metrics::{
            CognitiveComplexityCalculator, CyclomaticComplexityCalculator,
            HalsteadMetrics, MaintainabilityIndex, MetricsAnalyzer, SourceLinesOfCode,
        },
...
fn test_project_analysis() {
    ...
    // Analyze the project
    let results = pipeline.analyze_project(project_dir.to_str().unwrap()).unwrap();
...
fn test_real_world_crate_analysis() {
    ...
    let status = std::process::Command::new("git")
        .args(["clone", "--depth", "1", "https://github.com/serde-rs/json.git"])
        .arg(&crate_dir)
        .status()
    ...
    // Analyze the crate
    let results = pipeline.analyze_project(crate_dir.to_str().unwrap()).unwrap();
