...
async fn test_spec_generation_workflow() {
    ...
    let spec = parser.parse_specification(spec_text).await.unwrap();

    // Verify we found the expected entities and functions
    ...
    let generated = generator.generate_code(&spec).await.unwrap();

    // Verify we generated some files and resources
    ...
    println!("✅ All spec generation tests passed!");
}
