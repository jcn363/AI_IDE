...
fn test_halstead_metrics() {
    ...
    assert_eq!(halstead.unique_operators, 5, "Incorrect number of unique operators");
    assert_eq!(halstead.unique_operands, 6, "Incorrect number of unique operands");
    ...
    assert_eq!(halstead.program_vocabulary(), 11, "Incorrect program vocabulary");
...
impl TestStruct {
    ...
    assert!(sloc.logical >= 10 && sloc.logical <= 15, "Incorrect logical SLOC");
}
...
fn test_metrics_analyzer_integration() {
    ...
    assert!(
        result.findings.iter().any(|f| f.analysis_type == AnalysisType::CodeMetrics),
    ...
    assert!(func_metrics.is_some(), "No metrics found for complex_function");
    
    // The function should have a cyclomatic complexity > 1
    assert!(
        func_metrics.unwrap().message.contains("Cyclomatic Complexity: "),
...
fn test_high_complexity_detection() {
    ...
    let result = registry.analyze_code(code, "high_complexity_test.rs").unwrap();
