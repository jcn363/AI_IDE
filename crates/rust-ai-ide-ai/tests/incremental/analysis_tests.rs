...
fn test_incremental_analysis_skips_unchanged_files() {
    ...
    assert!(results.contains_key("file1.rs"), "Should analyze modified file1.rs");
}
...
fn test_incremental_analysis_dependencies() {
    ...
    let files = vec![
        ...
        ("file2.rs", "use super::file1::a; pub mod b { pub fn b() { super::a::a(); } }"),
    ];
    ...
    assert_eq!(results.len(), 2, "Should analyze both files when dependency changes");
}
...
fn test_analysis_state_serialization() {
    ...
    assert!(deserialized.get_dependents("file1.rs").unwrap().contains("file2.rs"));
}
...
fn test_cache_invalidation() {
    ...
    let results = analyze_files(&mut incremental, &files);
    assert_eq!(results.len(), 1, "Should re-analyze file after cache invalidation");
}
...
fn test_file_deletion() {
    ...
    assert!(!incremental.state().has_file("file2.rs"), "File2 should be removed from state");
}
...
fn test_file_renaming() {
    ...
    assert!(!incremental.state().has_file("old_name.rs"), "Old filename should be removed");
    assert!(incremental.state().has_file("new_name.rs"), "New filename should be added");
...
fn test_error_handling() {
    ...
    assert!(!results["error.rs"].is_ok(), "Should have error for invalid syntax");
    ...
    assert!(results["error.rs"].is_ok(), "Should not have error after fix");
}
...
fn test_file_addition() {
    ...
    assert!(incremental.state().has_file("a.rs"), "New file should be in state");
}
