...
use rust_ai_ide_ai1_semantic::{SemanticUnderstandingEngine, SemanticConfig};
use rust_ai_ide_ai1_architecture::{ArchitectureModernizationEngine, Codebase, Architecture};
...
impl CodeReviewEngine {
    ...
    pub async fn review_codebase(&self, codebase: &Codebase) -> Result<CodeReviewReport, ReviewError> {
        ...
        let report = CodeReviewReport {
            ...
            total_lines: codebase.files.iter().map(|f| f.content.lines().count()).sum(),
        };
    ...
    async fn review_architecture(&self, codebase: &Codebase) -> Result<Vec<ReviewFinding>, ReviewError> {
        ...
        let architecture_smells = self.architecture_engine.detect_architecture_smells(&architecture)
            .await
            .unwrap_or_default();
    ...
    fn analyze_semantic_issues(&self, analysis: &rust_ai_ide_ai1_semantic::SemanticAnalysis)
        ...
        for (module_name, module) in &architecture.modules {
            if module.complexity_score > 0.8 {
                findings.push(ReviewFinding {
                    ...
                    description: format!("Module {} has high complexity score ({})", module_name, module.complexity_score),
    ...
    fn generate_summary(findings: &[ReviewFinding]) -> ReviewSummary {
        ...
        let warning_count = findings.iter().filter(|f| matches!(f.severity, Severity::Warning)).count();
        ...
        ReviewSummary {
            ...
            warning_count,
            info_count,
            pass_rate: if total_findings > 0 { ((critical_count + error_count) as f64 / total_findings as f64) * 100.0 } else { 100.0 },
        }
    }
