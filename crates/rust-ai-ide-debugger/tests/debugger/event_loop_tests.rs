...
use rust_ai_ide_debugger::debugger::{
    Debugger, DebuggerCommand, DebuggerEvent, DebuggerState,
    DebuggerBackendTrait, DebuggerConfig, DebuggerError
};
...
impl DebuggerBackendTrait for MockDebuggerBackend {
    ...
    async fn get_variables(&self, _frame_id: Option<u32>) -> Result<Vec<crate::debugger::types::VariableInfo>, DebuggerError> {
        Ok(Vec::new())
    }
    
    async fn evaluate_expression(&self, _expression: &str, _frame_id: Option<u32>) -> Result<crate::debugger::types::VariableInfo, DebuggerError> {
        Err(DebuggerError::NotImplemented)
    }
}

// Helper function to create a test debugger instance
async fn create_test_debugger() -> (std::sync::Arc<tokio::sync::Mutex<Debugger>>, mpsc::UnboundedReceiver<DebuggerEvent>) {
...
async fn test_breakpoint_commands() {
    ...
    command_sender.send(DebuggerCommand::AddBreakpoint { 
    ...
    command_sender.send(DebuggerCommand::RemoveBreakpoint(1))
