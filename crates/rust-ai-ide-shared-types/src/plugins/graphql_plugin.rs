...
impl GeneratorPluginTrait for GraphQLGeneratorPlugin {
    fn name(&self) -> &str { "graphql-generator" }
    ...
    fn supported_formats(&self) -> Vec<String> {
        vec!["schema".to_string(), "federation".to_string(), "subscription".to_string()]
    }
    ...
    async fn generate(
        ...
        let schema_type = config.get("schema_type")
            .and_then(|v| v.as_str())
            .unwrap_or("basic");

        let generate_mutations = config.get("mutations")
            .unwrap_or(&serde_json::Value::Bool(true))
            .as_bool()
            .unwrap_or(true);

        let generate_subscriptions = config.get("subscriptions")
        ...
        match schema_type {
            "federation" => self.generate_federation_schema(&mut content, types, generate_mutations, generate_subscriptions),
...
impl GraphQLGeneratorPlugin {
    ...
    fn generate_federation_schema(&self, content: &mut String, types: &[ParsedType], with_mutations: bool, with_subscriptions: bool) {
        ...
        for rust_type in types {
            ...
            for field in &rust_type.fields {
                ...
                let nullable = if field.ty.contains("Option<") { "" } else { "!" };
    ...
    fn generate_subscription_schema(&self, content: &mut String, types: &[ParsedType]) {
        ...
        for rust_type in types {
            ...
            content.push_str(&format!("  {}: {}\n", self.to_subscription_name(rust_type), rust_type.name));
        }
        content.push_str("}\n\n");
    ...
    fn generate_basic_mutations(&self, content: &mut String, types: &[ParsedType]) {
        ...
        for rust_type in types {
            ...
            // Create mutation
            content.push_str(&format!("  create{}(input: {}Input!): {}\n",
                type_name, type_name, type_name));
            content.push_str(&format!("  update{}(id: ID!, input: {}Input!): {}\n",
                type_name, type_name, type_name));
    ...
    fn to_subscription_name(&self, rust_type: &ParsedType) -> String {
        ...
        format!("on{}", crate::utils::string_utils::to_pascal_case(type_name))
    }
}
