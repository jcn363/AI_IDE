...
impl StatePersistence {
    ...
    pub async fn create_checkpoint(
        &self,
        services: &HashMap<ServiceId, ServiceSnapshot>,
        operations: &[PendingOperation],
    ) -> SupervisorResult<CheckpointId> {
        ...
        match result {
            Ok(Ok(())) => Ok(id),
            Ok(Err(e)) => Err(e),
            Err(_) => Err(SupervisorError::persistence_error(
                "Checkpoint creation timeout".to_string(),
            )),
        }
    }
    ...
    pub async fn load_checkpoint(&self, id: &CheckpointId) -> SupervisorResult<StateSnapshot> {
        ...
        if calculated_checksum != checksum {
            return Err(SupervisorError::persistence_error(
    ...
    pub async fn load_latest_checkpoint(&self) -> SupervisorResult<StateSnapshot> {
        ...
        if calculated_checksum != checksum {
            return Err(SupervisorError::persistence_error(
                "Latest checkpoint checksum verification failed".to_string(),
            ));
        }
    ...
    pub async fn cleanup_old_checkpoints(&self, keep_latest: usize) -> SupervisorResult<usize> {
        ...
        let checkpoint_ids: Vec<String> = conn_guard
            .prepare("SELECT id FROM checkpoints ORDER BY timestamp DESC LIMIT -1 OFFSET ?")?
...
mod tests {
    ...
    async fn test_pending_operations() {
        ...
        persistence
            .store_pending_operation(&operation)
            .await
            .expect("Failed to store operation");

        let operations = persistence
            .load_pending_operations(None)
            .await
        ...
        persistence
            .remove_pending_operation(&operation.id)
            .await
            .expect("Failed to remove operation");

        let operations_after = persistence
            .load_pending_operations(None)
            .await
            .expect("Failed to load operations after removal");
