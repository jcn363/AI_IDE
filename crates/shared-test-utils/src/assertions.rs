...
pub mod performance {
    ...
    pub async fn assert_async_execution_time<F, Fut, T>(operation: F, max_duration: Duration) -> T
    where
        F: FnOnce() -> Fut,
    ...
    pub fn assert_file_format(path: &Path, format: &str) -> Result<(), TestError> {
        ...
        match format.to_lowercase().as_str() {
            ...
            _ => Err(TestError::Validation(
                crate::ValidationError::invalid_setup(format!("Unsupported format: {}", format)),
            )),
        }
    }
...
pub mod database {
    ...
    pub fn assert_tables_exist(conn: &Connection, expected_tables: &[&str]) {
        ...
        for expected_table in expected_tables {
            assert!(
                existing_tables.contains(&expected_table.to_string()),
                "Expected table '{}' not found. Existing tables: {:?}",
                expected_table,
                existing_tables
            );
        }
        ...
        if count == 0 {
            return Err(TestError::Validation(
                crate::ValidationError::invalid_setup(format!(
                    "No rows found in table '{}' matching conditions: {:?}",
                    table, conditions
                )),
            ));
        }
    ...
    pub fn assert_valid_json(json_str: &str, expected_fields: &[&str]) -> Result<(), TestError> {
        ...
        for field in expected_fields {
            assert!(
                ...
                "JSON does not contain expected field: {}",
                field
            );
        }
...
pub mod async_ops {
        ...
        T: PartialEq + Debug,
    {
        ...
        assert_eq!(
            result, expected,
            "Async operation returned unexpected result"
        );
        result
    }

    /// Assert that multiple async operations complete in order
    pub async fn assert_completion_order<F, Fut, T>(operations: Vec<F>) -> Vec<T>
    where
        F: Future<Output = T>,
        Fut: Future<Output = T>,
    {
...
impl<T: PartialEq + Debug> AssertionBuilder<T> {
    pub fn is_equal_to(self, expected: T) -> Self {
        assert_eq!(
            ...
            expected,
            "{}",
            self.format_error("Values are not equal")
        );
        self
    }
    ...
    pub fn is_not_equal_to(self, unexpected: T) -> Self {
        assert_ne!(
            ...
            unexpected,
            "{}",
            self.format_error("Values are equal")
        );
        self
    }
}
...
impl<T> AssertionBuilder<Option<T>> {
    ...
    pub fn is_none(self) -> Self {
        assert!(
            self.value.is_none(),
            "{}",
            self.format_error("Expected None, got Some")
        );
        self
    }
}
...
mod tests {
    ...
    fn test_performance_assertions() {
        let result = performance::assert_execution_time(|| 42, Duration::from_secs(1));
        assert_eq!(result, 42);
