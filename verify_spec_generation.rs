...
use rust_ai_ide_ai::spec_generation::{
    ...
    types::{SpecificationRequest, Specification},
...
async fn test_utils() -> Result<Specification, String> {
    ...
    println!("   ✓ Created test specification with {} entities and {} functions", 
        test_spec.entities.len(), 
        test_spec.functions.len()
    );
    assert!(test_spec.entities.len() > 0, "Test specification must have entities");
    Ok(test_spec)
}
...
async fn test_parser() -> Result<(), String> {
    ...
    let spec = parser.parse_specification(spec_text).await.map_err(|e| e.to_string())?;
    ...
    assert!(spec.entities.len() > 0, "Parsed specification must have entities");
    Ok(())
}
...
async fn test_generator(test_spec: &Specification) -> Result<(), String> {
    ...
    let generated_artifacts = code_generator.generate_code(test_spec).await.map_err(|e| e.to_string())?;
    println!("   ✓ Successfully generated code");
    println!("     - Generated {} files", generated_artifacts.files.len());
    println!("     - Generated {} resources", generated_artifacts.resources.len());
    assert!(generated_artifacts.files.len() > 0, "Code generator must produce files");
    Ok(())
}
...
async fn test_system(test_spec: &Specification) -> Result<(), String> {
    ...
    let result = intelligent_generator.generate_from_spec(&request).await.map_err(|e| e.to_string())?;
    ...
    if let Some(file) = result.files.first() {
        ...
        let preview = file.content.chars().take(MAX_PREVIEW_CHARS).collect::<String>();
        println!("{}", preview);
