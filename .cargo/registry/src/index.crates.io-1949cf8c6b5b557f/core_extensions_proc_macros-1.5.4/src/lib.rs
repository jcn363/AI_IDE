...
#[doc(hidden)]
#[proc_macro]
pub fn __priv_unwrap_bound(input_tokens: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let ty_tt = iter
        .next()
        .expect("__priv_unwrap_bound expected more tokens");
    ...
    parsing_shared::parse_path_and_args("__priv_unwrap_bound", &mut iter, args, |args| {
        args.extend(once(TokenTree::Group(Group::new(
...
#[doc(hidden)]
#[proc_macro]
pub fn __priv_rewrap_macro_parameters(
...
fn split_generics(input: TokenStream) -> TokenStream {
    ...
    SplitGenerics::new(input).split_generics(
        macro_invoc,
        TokenStream::new(),
        UnparsedPostGenerics {
