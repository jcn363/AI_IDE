...
impl SplitGenerics {
        ...
        I: IntoIterator<IntoIter = IntoIter, Item = TokenTree>,
    {
        ...
        let parsed_tt = input_tokens
            .next()
            .expect("skip_generics expected more tokens");
...
impl SplitGenerics {
    pub(crate) fn split_generics<P>(
        mut self,
        callback_macro: MacroInvocation,
        args: TokenStream,
        mut parsing_pgen: P,
    ) -> TokenStream
    where
    ...
    fn process_after_generics(&mut self, tt: TokenTree) -> Option<TokenTree> {
        match &tt {
            TokenTree::Ident(ident)
                if mmatches!(self.location, ParseLocation::AfterGenerics)
    ...
    fn process_generic_list(&mut self, tt: TokenTree) -> Option<TokenTree> {
        ...
        if let TokenTree::Punct(punct) = &tt {
            ...
            self.curr_is_joint =
                char == '-' || punct.spacing() == Spacing::Joint && char != '>' && char != '<';
