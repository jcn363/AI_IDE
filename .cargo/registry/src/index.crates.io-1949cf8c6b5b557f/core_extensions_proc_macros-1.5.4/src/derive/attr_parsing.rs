...
fn parse_helper_attribute_contents<'a, T: AttrParsing<'a>>(
    ...
    if let Some(_) = input.peek_parse(Token!(where))? {
        ...
        if !input.is_empty() {
            ...
            loop {
                this.extra_predicates
                    .push(input.parse::<syn::WherePredicate>()?);
                if input.is_empty() {
                    break;
                }
                input.parse::<Token!(,)>()?;
...
pub(crate) fn check_is_container(ctx: &ParseCtx<'_>, sp: &dyn spanned::Spanned) -> syn::Result<()> {
...
pub(crate) fn check_is_field<'a>(
    ...
    match ctx {
        ...
        _ => Err(syn::Error::new(
            sp.span(),
            "Can only use this attribute on a field",
        )),
    }
}
...
pub(crate) fn check_is_variant_or_field(
    ...
    if mmatches!(ctx, ParseCtx::Container) {
        Err(syn::Error::new(
            sp.span(),
            "Can only use this attribute on variants of fields",
        ))
    } else {
        Ok(())
    }
