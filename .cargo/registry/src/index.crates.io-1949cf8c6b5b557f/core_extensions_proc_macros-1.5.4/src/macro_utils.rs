use crate::{
    macro_utils_shared::{
        ...
        parse_bounded_range_param,
        parse_check_punct,
        parse_count_param,
        parse_ident,
        parse_int_or_range_param,
        parse_keyword,
        parse_parentheses,
    },
    mmatches,
...
pub(crate) fn gen_ident_range(tokens: TokenStream) -> crate::Result<TokenStream> {
    ...
    let idents = try_!(gen_ident_range_just_idents(&mut iter, |x| {
        parse_bounded_range_param(x)
    }));
...
pub(crate) fn macro_attr(attr: TokenStream, item: TokenStream) -> crate::Result<TokenStream> {
    ...
    let (bang, more_tokens) = match macro_.terminator {
        Some(TokenTree::Punct(punct)) if punct.as_char() == '!' => (punct, true),
        Some(tt) => return Err(crate::Error::one_tt(tt.span(), "expected a `!`")),
        None => {
...
fn parse_for_zip(iter: IntoIter) -> crate::Result<ZipArgs> {
    ...
    if finite_arg_count == 0 {
        Err(crate::Error::one_tt(
            macro_span(),
            "Expected at least one finite list",
        ))
    } else {
        Ok(ZipArgs {
            iters: groups,
