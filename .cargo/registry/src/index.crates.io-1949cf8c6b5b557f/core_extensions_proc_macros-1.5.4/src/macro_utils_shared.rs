...
pub(crate) fn parse_int_or_range_param(input: &mut Peekable<IntoIter>) -> crate::Result<RangeB> {
    let (start, start_span) = try_!(parse_start_bound(&mut *input));
...
fn parse_range_operator_opt(input: &mut Peekable<IntoIter>) -> crate::Result<Option<RangeType>> {
    match input.next() {
        Some(TokenTree::Punct(punct))
            if punct.as_char() == '.' && mmatches!(punct.spacing(), Spacing::Joint) =>
        {
...
pub(crate) fn parse_path_and_span<I>(iter: I) -> crate::Result<PathAndSpan>
where
    I: IntoIterator<Item = TokenTree>,
{
