...
fn expand_equiv_class(cls: &[N]) -> Vec<N> {
    ...
    for e in cls {
        ...
        match e {
            ...
            N::u32(v) => {
                ret.extend_from_slice(&[N::u32(*v), N::u64(*v as u64), N::u128(*v as u128)])
            }
            ...
            N::i32(v) => {
                ret.extend_from_slice(&[N::i32(*v), N::i64(*v as i64), N::i128(*v as i128)])
            }
...
fn test_rational_using_primitives() {
    fn expand_equiv_class_ratio(cls: &[N]) -> Vec<N> {
        ...
        for e in cls {
            ...
            match e {
                ...
                #[cfg(feature = "num-bigint")]
                N::i128(v) => ret.extend_from_slice(&[
                    N::i128(*v),
                    N::r128(Ratio::from(*v)),
                    N::rbig(Ratio::from(BigInt::from(*v))),
                ]),
                N::isize(v) => ret.extend_from_slice(&[N::isize(*v), N::rsize(Ratio::from(*v))]),
                #[cfg(feature = "num-bigint")]
                N::ibig(v) => {
                    ret.extend_from_slice(&[N::ibig(v.clone()), N::rbig(Ratio::from(v.clone()))])
                }
                _ => {}
            }
        }
    ...
    let numbers: Vec<_> = NUMBERS
        .iter()
        .map(|cls| expand_equiv_class_ratio(cls))
        .collect();
...
fn test_rational() {
    ...
    let ratio_coeffs = [
        ...
        (
            N::i32(-(1 << 22) - 1),
            N::i32(1 << 23),
            Some(N::f32(-0.5 - 2f32.powi(-23))),
        ),
        (
            N::i64(-(1 << 52) - 1),
            N::i64(1 << 53),
            Some(N::f64(-0.5 - 2f64.powi(-53))),
        ),
    ...
    fn expand_equiv_class_ratio(coeffs: &(N, N, Option<N>)) -> Vec<N> {
        ...
        match coeffs {
            ...
            (N::ibig(num), N::ibig(den), _) => {
                ret.extend_from_slice(&[N::rbig(Ratio::new(num.clone(), den.clone()))])
            }
            _ => unreachable!(),
        };

        match coeffs.2 {
            Some(N::f32(v)) => ret.extend_from_slice(&[N::f32(v), N::f64(v as f64)]),
    ...
    fn expand_equiv_class_ratio(cls: &[N]) -> Vec<N> {
        ...
        for e in cls {
            ...
            match e {
                ...
                N::f32(v) => ret.extend_from_slice(&[
                    N::f32(*v),
                    N::c32(Complex::from(*v)),
                    N::f64(*v as f64),
                    N::c64(Complex::from(*v as f64)),
                ]),
                _ => {}
            }
        }
...
fn test_complex() {
    ...
    fn expand_equiv_class_ratio(coeffs: &(N, N)) -> Vec<N> {
        ...
        match coeffs {
            ...
            (N::f64(re), N::f64(im)) if im == &0. => {
                ret.extend_from_slice(&[N::c64(Complex::new(*re, *im)), N::f64(*re as f64)])
            }
            (N::f64(re), N::f64(im)) => ret.push(N::c64(Complex::new(*re, *im))),
            (_, _) => unreachable!(),
        };
        ret
    }
