...
fn test_foo() {
    imp::assert_eq!(
        <Foo as imp::TryFromBytes>::try_read_from_bytes(&[0]),
        imp::Ok(Foo::A)
    );
...
fn test_bar() {
    imp::assert_eq!(
        <Bar as imp::TryFromBytes>::try_read_from_bytes(&[0, 0]),
        imp::Ok(Bar::A)
    );
...
fn test_fieldless_but_not_unit_only() {
    ...
    imp::assert!(
        <FieldlessButNotUnitOnly as imp::TryFromBytes>::try_read_from_bytes(&[0xFF; SIZE][..])
            .is_err()
    );
}
...
fn test_has_fields() {
    ...
    let bytes: [u8; SIZE] = ::zerocopy::transmute!(HasFields::B {
...
fn test_has_fields_aligned() {
    ...
    let wrap = |has_fields| BytesOfHasFieldsAligned {
        has_fields,
        padding: [0; 8],
    };
...
fn test_has_fields_primitive_aligned() {
    ...
    let wrap = |has_fields| BytesOfHasFieldsPrimitiveAligned {
        has_fields,
        padding: [0; 8],
    };
