...
fn utf8_extra_bytes() {
    ...
    for c in 0..256 {
        assert_eq!(
            (c as u8).extra_utf8_bytes_unchecked(),
            match c {
...
fn utf16_extra_unit() {
    for c in 0..0x1_00_00 {
        assert_eq!(
            (c as u16).utf16_needs_extra_unit(),
            match c {
...
fn from_utf16_tuple() {
    ...
    for u in 0xdc00..0xe000 {
        ...
        assert_eq!(
            char::from_utf16_tuple((u, doesnt_matter)),
    ...
    for u in 0xd800..0xdc00 {
        ...
        assert_eq!(
            char::from_utf16_tuple((u, Some(u - 0x2ff))),
...
fn from_utf16_slice_start() {
    ...
    for u in 0xd800..0xdc00 {
        ...
        assert_eq!(
            char::from_utf16_slice_start(&buf[..pass]),
            Err(SecondIsNotTrailingSurrogate)
        );
    }
    for u in 0xdc00..0xe000 {
...
#[test]
fn utf8_codepoint_is_too_high() {
    assert_eq!(
        kind(Utf8Char::from_array([0xf4, 0x90, 0x80, 0x80])),
        Err(TooHighCodepoint)
    );
    assert_eq!(
        kind(char::from_utf8_array([0xf4, 0x90, 0x80, 0x80])),
        Err(TooHighCodepoint)
    );
    assert_eq!(
        kind(Utf8Char::from_slice_start(&[0xf4, 0x90, 0x80, 0x80])),
        Err(TooHighCodepoint)
    );
    assert_eq!(
        kind(char::from_utf8_slice_start(&[0xf4, 0x90, 0x80, 0x80])),
        Err(TooHighCodepoint)
    );

    assert_eq!(
        kind(Utf8Char::from_array([0xf4, 0xa4, 0xb0, 0x9f])),
        Err(TooHighCodepoint)
    );
    assert_eq!(
        kind(char::from_utf8_array([0xf4, 0xa4, 0xb0, 0x9f])),
        Err(TooHighCodepoint)
    );
    assert_eq!(
        kind(Utf8Char::from_slice_start(&[0xf4, 0xa4, 0xb0, 0x9f])),
        Err(TooHighCodepoint)
    );
    assert_eq!(
        kind(char::from_utf8_slice_start(&[0xf4, 0xa4, 0xb8, 0x9f])),
        Err(TooHighCodepoint)
    );

    assert_eq!(
        kind(Utf8Char::from_array([0xf5, 0x88, 0x99, 0xaa])),
        Err(NonUtf8Byte)
    );
    assert_eq!(
        kind(char::from_utf8_array([0xf5, 0xaa, 0xbb, 0x88])),
        Err(NonUtf8Byte)
    );
    assert_eq!(
        kind(Utf8Char::from_slice_start(&[0xf5, 0x99, 0xaa, 0xbb])),
        Err(NonUtf8Byte)
    );
    assert_eq!(
        kind(char::from_utf8_slice_start(&[0xf5, 0xbb, 0x88, 0x99])),
        Err(NonUtf8Byte)
    );
}

#[test]
fn utf8_codepoint_is_utf16_reserved() {
    assert_eq!(
        kind(Utf8Char::from_array([0xed, 0xa0, 0x80, 0xff])),
        Err(Utf16ReservedCodepoint)
    );
    assert_eq!(
        kind(char::from_utf8_array([0xed, 0xa0, 0x8f, 0x00])),
        Err(Utf16ReservedCodepoint)
    );
    assert_eq!(
        kind(Utf8Char::from_slice_start(&[0xed, 0xa0, 0xbe, 0xa5])),
        Err(Utf16ReservedCodepoint)
    );
    assert_eq!(
        kind(char::from_utf8_slice_start(&[0xed, 0xa0, 0xbf])),
        Err(Utf16ReservedCodepoint)
    );
    assert_eq!(
        kind(Utf8Char::from_array([0xed, 0xbf, 0x80, 0xff])),
        Err(Utf16ReservedCodepoint)
    );
    assert_eq!(
        kind(char::from_utf8_array([0xed, 0xbf, 0x8f, 0x00])),
        Err(Utf16ReservedCodepoint)
    );
    assert_eq!(
        kind(Utf8Char::from_slice_start(&[0xed, 0xbf, 0xbe, 0xa5])),
        Err(Utf16ReservedCodepoint)
    );
    assert_eq!(
        kind(char::from_utf8_slice_start(&[0xed, 0xbf, 0xbf])),
        Err(Utf16ReservedCodepoint)
    );
}

#[test]
fn utf8_first_is_continuation_byte() {
    for first in 0x80..0xc0 {
        let arr = [first, first << 2, first << 4, first << 6];
        assert_eq!(
            kind(Utf8Char::from_array(arr)),
            Err(UnexpectedContinuationByte)
        );
        assert_eq!(
            kind(char::from_utf8_array(arr)),
            Err(UnexpectedContinuationByte)
        );
        let len = (1 + first % 3) as usize;
        assert_eq!(
            kind(Utf8Char::from_slice_start(&arr[..len])),
            Err(UnexpectedContinuationByte)
        );
        assert_eq!(
            kind(char::from_utf8_slice_start(&arr[..len])),
            Err(UnexpectedContinuationByte)
        );
    }
}
    ...
    for first in 0xc2..0xf4 {
        ...
        for corrupt in (1..extra).rev() {
            for &bad in &[0x00, 0x3f, 0x40, 0x7f, 0xc0, 0xff] {
                ...
                assert_eq!(
                    kind(Utf8Char::from_array(arr)),
                    Err(InterruptedSequence),
                    "{:?}",
                    arr
                );
                assert_eq!(kind(char::from_utf8_array(arr)), Err(InterruptedSequence));
                let slice = if first & 1 == 0 {
                    &arr[..1 + extra]
                } else {
                    &arr
                };
                assert_eq!(
                    kind(Utf8Char::from_slice_start(slice)),
                    Err(InterruptedSequence),
                    "{:?}",
                    slice
                );
                assert_eq!(
                    kind(char::from_utf8_slice_start(slice)),
                    Err(InterruptedSequence)
                );
            }
        }
