...
fn utf8charmerger() {
    ...
    assert_eq!(
        format!("{:?}", &iter),
        format!(
            "Utf8CharMerger {{ buffered: [], inner: {:?} }}",
            slice.iter()
        )
    );

    assert_eq!(
        iter.next().map(|v| v.map_err(|e| e.kind())),
        Some(Err(InterruptedSequence))
    );
    assert_eq!(iter.size_hint(), (0, Some(5)));
    assert_eq!(
        format!("{:?}", &iter),
        format!(
            "Utf8CharMerger {{ buffered: [161, 146, 88], inner: {:?} }}",
            slice[4..].iter()
        )
    );
    ...
    assert_eq!(
        iter.next().map(|v| v.map_err(|e| e.kind())),
        Some(Err(UnexpectedContinuationByte))
    );
...
fn utf8chardecoder() {
    ...
    match iter.next() {
        ...
        wrong => panic!(
            "Expected Some((0, Err(TooHighCodepoint), 1), got {:?}",
            wrong
        ),
    }
    assert_eq!(
...
fn utf16charmerger() {
    ...
    assert_eq!(
        format!("{:?}", &iter),
        format!(
            "Utf16CharMerger {{ buffered: None, inner: {:?} }}",
            slice.iter()
        )
    );
    ...
    assert_eq!(
        format!("{:?}", &iter),
        format!(
            "Utf16CharMerger {{ buffered: Some(120), inner: {:?} }}",
            slice[2..].iter()
        )
    );
...
fn utf8charsplitter_read_all_sizes() {
    ...
    for size in 2..6 {
        ...
        for (offset, part) in s.as_bytes().chunks(size).enumerate() {
            let read_to = if part.len() == size {
                (offset + 1) * size
            } else {
                buf.len()
            };
            assert_eq!(
                reader.read(&mut buf[offset * size..read_to]).unwrap(),
                part.len()
            );
            assert_eq!(
                &buf[..offset * size + part.len()],
                &s.as_bytes()[..offset * size + part.len()]
            );
        }
...
fn utf8charsplitter_alternate_iter_read() {
    ...
    for n in 0..2 {
        ...
        for (i, byte) in s.bytes().enumerate() {
            let until_next = s.as_bytes()[i..]
                .iter()
                .take_while(|&b| (b >> 6) == 0b10u8)
                .count();
