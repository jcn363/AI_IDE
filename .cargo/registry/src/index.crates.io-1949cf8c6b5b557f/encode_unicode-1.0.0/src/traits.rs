...
impl CharExt for char {
    ...
    fn from_utf8_slice_start(src: &[u8]) -> Result<(Self, usize), Utf8Error> {
        ...
        let bytes = match first.extra_utf8_bytes() {
            ...
            Ok(extra) if extra >= src.len() => return Err(Utf8Error { kind: TooFewBytes }),
            Ok(extra) => &src[..=extra],
        };
        if bytes.iter().skip(1).any(|&b| (b >> 6) != 0b10) {
            ...
            match char::from_u32_detailed(merge_nonascii_unchecked_utf8(bytes)) {
                Ok(c) => Ok((c, bytes.len())),
                Err(CodepointError::Utf16Reserved) => Err(Utf8Error {
                    kind: Utf16ReservedCodepoint,
                }),
                Err(CodepointError::TooHigh) => Err(Utf8Error {
