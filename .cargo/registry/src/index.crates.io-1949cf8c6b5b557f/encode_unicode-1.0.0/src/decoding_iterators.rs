...
impl<B: Borrow<u8>, I: Iterator<Item = B>, T: IntoIterator<IntoIter = I, Item = B>> From<T>
    for Utf8CharMerger<B, I>
{
    fn from(t: T) -> Self {
        Utf8CharMerger {
            iter: t.into_iter(),
...
impl<B: Borrow<u8>, I: Iterator<Item = B>> Utf8CharMerger<B, I> {
    ...
    fn extra(&mut self, bytes: &mut [u8; 4], len: usize) -> Result<(), Utf8Error> {
        ...
        debug_assert_eq!(
            self.after_err_leftover, 0,
            "first: {:#02x}, stack: {:?}",
            bytes[0], self.after_err_stack
        );
        for i in 1..len {
...
impl<B: Borrow<u8>, I: Iterator<Item = B>> Iterator for Utf8CharMerger<B, I> {
        ...
        unsafe {
            ...
            let ok = match first {
                ...
                0b1000_0000..=0b1011_1111 => {
                    // continuation byte
                    Err(Utf8Error {
                        kind: UnexpectedContinuationByte,
                    })
                }
                0b1100_0000..=0b1100_0001 => {
                    // 2 and overlong
                    Err(Utf8Error { kind: NonUtf8Byte })
                }
                0b1111_0101..=0b1111_0111 => {
                    // 4 and too high codepoint
                    Err(Utf8Error { kind: NonUtf8Byte })
                }
                0b1111_1000..=0b1111_1111 => Err(Utf8Error { kind: NonUtf8Byte }),
            };
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        let max = iter_max.and_then(|max| max.checked_add(self.after_err_leftover as usize));
        (min, max)
    }
}
...
impl<B: Borrow<u16>, I: Iterator<Item = B>, T: IntoIterator<IntoIter = I, Item = B>> From<T>
    for Utf16CharMerger<B, I>
{
    fn from(t: T) -> Self {
        Utf16CharMerger {
            iter: t.into_iter(),
