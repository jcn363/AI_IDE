...
fn display(name_pad: usize, name: Cow<str>, r: Distribution, err: Option<io::Error>) {
    let c = r.utf8;
    ...
    println!(
        "{:>6$}: bytes: {:7}, UTF-8 distribution: [{:7}, {:6}, {:6}, {:6}]",
        name, r.bytes, s[0], s[1], s[2], s[3], name_pad
    );
    println!(
        "{5:6$}  chars: {:7}, UTF-8 percentages:  [{:>6.2}%, {:>5.2}%, {:>5.2}%, {:>5.2}%]",
