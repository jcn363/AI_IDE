...
use runner::{
    EventLoopRunner,
    EventLoopRunnerShared,
};

use super::{
    dpi::hwnd_dpi,
    util::get_system_metrics_for_dpi,
};
    ...
    let callback = || match msg {
        ...
        win32wm::WM_EXITSIZEMOVE => {
            ...
            if state.dragging {
                ...
                let _ =
                    unsafe { PostMessageW(Some(window), WM_LBUTTONUP, WPARAM::default(), lparam) };
            }
        ...
        win32wm::WM_WINDOWPOSCHANGING => {
            ...
            if let Some(ref mut fullscreen) = window_state.fullscreen {
                ...
                let new_rect = if (window_pos.flags & NOMOVE_OR_NOSIZE)
            ...
            let (allow_resize, is_decorated) = {
                ...
                (
                    window_state.fullscreen.is_none()
                        && !window_flags.contains(WindowFlags::MAXIMIZED),
