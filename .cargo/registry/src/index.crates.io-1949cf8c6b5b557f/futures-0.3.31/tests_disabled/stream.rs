...
fn from_err() {
    assert_done(
        || err_list().err_into().collect::<Vec<_>>(),
        Err(FromErrTest(3)),
    );
}
...
fn and_then() {
    assert_done(
        || list().and_then(|a| Ok(a + 1)).collect(),
        Ok(vec![2, 3, 4]),
    );
    assert_done(
        || {
            list()
                .and_then(|a| err::<i32, u32>(a as u32))
                .collect::<Vec<_>>()
        },
        Err(1),
    );
...
fn then() {
    assert_done(
        || list().then(|a| a.map(|e| e + 1)).collect(),
        Ok(vec![2, 3, 4]),
    );
}
...
fn or_else() {
    assert_done(
        || err_list().or_else(|a| ok::<i32, u32>(a as i32)).collect(),
        Ok(vec![1, 2, 3]),
    );
}
...
fn flatten() {
    assert_done(
        || list().map(|_| list()).flatten().collect(),
...
fn skip_while() {
    assert_done(
        || list().skip_while(|e| Ok(*e % 2 == 1)).collect(),
        Ok(vec![2, 3]),
    );
}
#[test]
...
fn take_while() {
    assert_done(
        || list().take_while(|e| Ok(*e < 3)).collect(),
        Ok(vec![1, 2]),
    );
}
...
fn zip() {
    assert_done(
        || list().zip(list()).collect(),
        Ok(vec![(1, 1), (2, 2), (3, 3)]),
    );
    assert_done(
        || list().zip(list().take(2)).collect(),
        Ok(vec![(1, 1), (2, 2)]),
    );
    assert_done(
        || list().take(2).zip(list()).collect(),
        Ok(vec![(1, 1), (2, 2)]),
    );
    assert_done(|| err_list().zip(list()).collect::<Vec<_>>(), Err(3));
    assert_done(
        || list().zip(list().map(|x| x + 1)).collect(),
        Ok(vec![(1, 2), (2, 3), (3, 4)]),
    );
}
...
fn wait() {
    assert_eq!(
        block_on_stream(list()).collect::<Result<Vec<_>, _>>(),
        Ok(vec![1, 2, 3])
    );
}
...
fn forward() {
    let v = Vec::new();
    let v = block_on(iter_ok::<_, Never>(vec![0, 1]).forward(v))
        .unwrap()
        .1;
    assert_eq!(v, vec![0, 1]);

    let v = block_on(iter_ok::<_, Never>(vec![2, 3]).forward(v))
        .unwrap()
        .1;
...
fn inspect() {
    let mut seen = vec![];
    assert_done(
        || list().inspect(|&a| seen.push(a)).collect(),
        Ok(vec![1, 2, 3]),
    );
