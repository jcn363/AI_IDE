    ...
    fn to_str_lossy(&self) -> Cow<'_, str> {
        match utf8::validate(self.as_bytes()) {
            Ok(()) => {
                // SAFETY: This is safe because of the guarantees provided by
                // utf8::validate.
                unsafe { Cow::Borrowed(str::from_utf8_unchecked(self.as_bytes())) }
            }
            Err(err) => {
...
impl<'h, 'n> Find<'h, 'n> {
    fn new(haystack: &'h [u8], needle: &'n [u8]) -> Find<'h, 'n> {
        Find {
            it: memmem::find_iter(haystack, needle),
            haystack,
            needle,
        }
    }
...
impl<'a> Fields<'a> {
    fn new(bytes: &'a [u8]) -> Fields<'a> {
        Fields {
            it: bytes.fields_with(char::is_whitespace),
        }
    }
}
...
impl<'h, 's> SplitN<'h, 's> {
    fn new(haystack: &'h [u8], splitter: &'s [u8], limit: usize) -> SplitN<'h, 's> {
...
impl<'h, 's> SplitNReverse<'h, 's> {
    fn new(haystack: &'h [u8], splitter: &'s [u8], limit: usize) -> SplitNReverse<'h, 's> {
