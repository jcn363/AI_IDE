        ...
        match self.state {
            Start => {
                ...
                self.state = match byte {
                    0x21..=0x5B | 0x5D..=0x7E => return Some(char::from(byte)),
                    b'\0' => SpecialEscape('0'),
                    b'\n' => SpecialEscape('n'),
                    b'\r' => SpecialEscape('r'),
                    b'\t' => SpecialEscape('t'),
...
impl<I: Iterator<Item = char>> UnescapeBytes<I> {
    pub(crate) fn new<T: IntoIterator<IntoIter = I>>(t: T) -> UnescapeBytes<I> {
        UnescapeBytes {
            it: t.into_iter(),
            state: UnescapeState::Start,
        }
    }
}
            ...
            match self.state {
                ...
                HexSecond(first) => {
                    ...
                    match second {
                        ...
                        ch => {
                            // An invalid escape sequence unescapes as itself.
                            self.state = UnescapeState::bytes2(&[b'x'], first, ch);
                            return Some(b'\\');
                        }
                    }
                }
