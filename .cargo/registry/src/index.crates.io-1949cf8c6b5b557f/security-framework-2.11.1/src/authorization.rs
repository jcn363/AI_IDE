...
use security_framework_sys::{
    authorization as sys,
    base::errSecConversionError,
};
use std::{
    ffi::{
        CStr,
...
mod tests {
    ...
    fn create_credentials_env() -> Result<AuthorizationItemSetStorage> {
        let set = AuthorizationItemSetBuilder::new()
            .add_string(
                "username",
                std::env::var("USER").expect("You must set the USER environment variable"),
            )?
            .add_string(
                "password",
                std::env::var("PASSWORD").expect("You must set the PASSWORD environment varible"),
            )?
            .build();
