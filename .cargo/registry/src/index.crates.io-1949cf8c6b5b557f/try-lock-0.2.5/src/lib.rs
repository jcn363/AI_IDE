...
impl<T> TryLock<T> {
    ...
    pub fn try_lock(&self) -> Option<Locked<T>> {
        unsafe { self.try_lock_explicit_unchecked(Ordering::Acquire, Ordering::Release) }
    }
    ...
    pub fn try_lock_order(
        ...
        unsafe { self.try_lock_explicit_unchecked(lock_order, unlock_order) }
    }
