...
fn bench_cpuinfo(c: &mut Criterion) {
    c.bench_function("CpuInfo::new", |b| {
        b.iter(|| black_box(CpuInfo::new().unwrap()))
    });

    let cpuinfo = black_box(CpuInfo::new().unwrap());
    c.bench_function("CpuInfo::get_info", |b| {
        b.iter(|| black_box(cpuinfo.get_info(0)))
    });
    ...
    c.bench_function("CpuInfo::physical_id", |b| {
        b.iter(|| cpuinfo.physical_id(0))
    });
