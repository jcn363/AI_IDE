...
fn test_proc_pagemap() {
    ...
    let stack_map = maps
        .iter()
        .find(|m| matches!(m.pathname, MMapPath::Stack))
        .unwrap();
...
fn test_proc_auxv() {
    ...
    for (k, v) in auxv {
        ...
        match k {
            ...
            3 => println!(
                "Address of the program headers of the executable: 0x{:x}",
                v
            ),
...
fn test_limits() {
    ...
    assert_eq!(
        lim.current,
        limits.max_pending_signals.soft_limit.as_limit()
    );
    assert_eq!(
        lim.maximum,
        limits.max_pending_signals.hard_limit.as_limit()
    );
    ...
    assert_eq!(
        lim.current,
        limits.max_realtime_priority.soft_limit.as_limit()
    );
    assert_eq!(
        lim.maximum,
        limits.max_realtime_priority.hard_limit.as_limit()
    );
    ...
    assert_eq!(
        lim.current,
        limits.max_realtime_timeout.soft_limit.as_limit()
    );
    assert_eq!(
        lim.maximum,
        limits.max_realtime_timeout.hard_limit.as_limit()
    );
}
