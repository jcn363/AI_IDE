...
pub mod sys;
pub use crate::sys::kernel::{
    BuildInfo as KernelBuildInfo,
    Type as KernelType,
    Version as KernelVersion,
};
...
impl Current for KernelConfig {
    ...
    #[cfg_attr(
        not(feature = "flate2"),
        doc = "The flate2 feature is NOT currently enabled"
    )]
    fn current() -> ProcResult<Self> {
        let reader: Box<dyn Read> =
            if Path::new(PROC_CONFIG_GZ).exists() && cfg!(feature = "flate2") {
                #[cfg(feature = "flate2")]
                {
...
mod tests {
    ...
    fn test_esrch() {
        ...
        let p =
            crate::process::Process::new(command.id() as i32).expect("Failed to create Process");
    ...
    fn test_meminfo() {
        ...
        if kernel >= KernelVersion::new(2, 6, 19)
            && config.as_ref().map_or(false, |KernelConfig(cfg)| {
                cfg.contains_key("CONFIG_HIGHMEM")
            })
        {
        ...
        if kernel >= KernelVersion::new(2, 6, 27)
            && config.as_ref().map_or(false, |KernelConfig(cfg)| {
                cfg.contains_key("CONFIG_QUICKLIST")
            })
        {
        ...
        if config.as_ref().map_or(true, |KernelConfig(cfg)| {
            cfg.contains_key("CONFIG_HUGETLB_PAGE")
        }) {
