...
variant_from_value!(IUnknown, VT_UNKNOWN, punkVal, |v: IUnknown| {
    ManuallyDrop::new(Some(v))
});
...
impl TryFrom<&PROPVARIANT> for IUnknown {
    ...
    fn try_from(from: &PROPVARIANT) -> Result<Self> {
        unsafe {
            if from.Anonymous.Anonymous.vt == VT_UNKNOWN
                && !from.Anonymous.Anonymous.Anonymous.punkVal.is_none()
            {
...
variant_from_value!(bool, VT_BOOL, boolVal, |v: bool| VARIANT_BOOL(if v {
    -1
} else {
    0
}));
        ...
        Self {
            Anonymous: PROPVARIANT_0 {
                Anonymous: ManuallyDrop::new(PROPVARIANT_0_0 {
                    ...
                    wReserved2: 0,
                    wReserved3: 0,
                    Anonymous: PROPVARIANT_0_0_0 { uhVal: value },
                }),
            },
        }
    }
...
impl TryFrom<&PROPVARIANT> for IDispatch {
    ...
    fn try_from(from: &PROPVARIANT) -> windows_core::Result<Self> {
        unsafe {
            if from.Anonymous.Anonymous.vt == VT_DISPATCH
                && !from.Anonymous.Anonymous.Anonymous.pdispVal.is_none()
            {
