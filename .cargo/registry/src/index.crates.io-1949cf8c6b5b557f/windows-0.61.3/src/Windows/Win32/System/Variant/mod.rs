...
pub unsafe fn ClearVariantArray(pvars: &mut [VARIANT]) {
    ...
    unsafe {
        ClearVariantArray(
            core::mem::transmute(pvars.as_ptr()),
            pvars.len().try_into().unwrap(),
        )
    }
}
#[inline]
...
pub unsafe fn InitVariantFromFileTimeArray(
    prgft: Option<&[super::super::Foundation::FILETIME]>,
) -> windows_core::Result<VARIANT> {
...
pub unsafe fn InitVariantFromVariantArrayElem(
    ...
    unsafe {
        let mut result__ = core::mem::zeroed();
        InitVariantFromVariantArrayElem(core::mem::transmute(varin), ielem, &mut result__)
            .map(|| core::mem::transmute(result__))
    }
}
#[inline]
...
pub unsafe fn VariantToString(
    ...
    unsafe {
        VariantToString(
            core::mem::transmute(varin),
            core::mem::transmute(pszbuf.as_ptr()),
            pszbuf.len().try_into().unwrap(),
        )
        .ok()
    }
}
...
#[cfg(all(feature = "Win32_System_Com", feature = "Win32_System_Ole"))]
#[inline]
pub unsafe fn VariantToStringWithDefault<P1>(
    varin: *const VARIANT,
    pszdefault: P1,
) -> windows_core::PCWSTR
where
