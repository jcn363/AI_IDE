...
{
    windows_link::link!("ualapi.dll" "system" fn UalRegisterProduct(wszproductname : windows_core::PCWSTR, wszrolename : windows_core::PCWSTR, wszguid : windows_core::PCWSTR) -> windows_core::HRESULT);
    unsafe {
        UalRegisterProduct(
            wszproductname.param().abi(),
            wszrolename.param().abi(),
            wszguid.param().abi(),
        )
        .ok()
    }
