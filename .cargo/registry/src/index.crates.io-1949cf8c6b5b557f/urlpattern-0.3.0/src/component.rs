...
use crate::{
    canonicalize_and_process::escape_pattern_string,
    matcher::{
        InnerMatcher,
        Matcher,
    },
    ...
    regexp::RegExp,
    tokenizer::is_valid_name_codepoint,
    Error,
};
use std::fmt::Write;
...
impl<R: RegExp> Component<R> {
    ...
    {
        let part_list =
            crate::parser::parse_pattern_string(input.unwrap_or("*"), &options, encoding_callback)?;
        ...
        Ok(Component {
            ...
            matcher,
            has_regexp_group: part_list.iter().any(|part| part.kind == PartType::Regexp),
        })
    }
    ...
    for (i, part) in part_list.iter().enumerate() {
            ...
            && matches!(next_part, Some(Part { prefix, suffix, .. }) if prefix.is_empty() && suffix.is_empty())
        {
            ...
            if next_part.kind == PartType::FixedText {
                ...
                needs_grouping = next_part.name.chars().next().unwrap().is_ascii_digit();
            }
        }
        if !needs_grouping
        ...
        match part.kind {
            ...
            PartType::SegmentWildcard if !custom_name => {
                write!(result, "({})", options.generate_segment_wildcard_regexp()).unwrap()
            }
...
fn generate_matcher<R: RegExp>(
    ...
    let inner = match part_list {
        ...
        [part] if part.kind == PartType::FullWildcard && part.modifier == PartModifier::None => {
            prefix += &part.prefix;
        ...
        // prefix or suffix, we can use a simple wildcard matcher.
        [part] if part.kind == PartType::SegmentWildcard && part.modifier == PartModifier::None => {
            prefix += &part.prefix;
            if !part.suffix.is_empty() {
                suffix = format!("{}{suffix}", part.suffix);
            }
        ...
        part_list => {
            let (regexp_string, _) = generate_regular_expression_and_name_list(part_list, options);
            let regexp = R::parse(&regexp_string, flags).map_err(Error::RegExp);
