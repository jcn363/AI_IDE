...
impl Tokenizer {
    ...
    fn process_tokenizing_error(
        ...
        if self.policy == TokenizePolicy::Strict {
            ...
            self.add_token_with_default_len(TokenType::InvalidChar, next_pos, value_pos);
            Ok(())
        }
    }
...
pub fn tokenize(input: &str, policy: TokenizePolicy) -> Result<Vec<Token>, Error> {
    ...
    while tokenizer.index < tokenizer.input.len() {
        ...
        if tokenizer.code_point == Some(':') {
            ...
            while name_pos < tokenizer.input.len() {
                ...
                let valid_codepoint =
                    is_valid_name_codepoint(tokenizer.code_point.unwrap(), first_code_point);
                if !valid_codepoint {
                    break;
                }
            ...
            tokenizer.add_token_with_default_len(TokenType::Name, name_pos, name_start);
            continue;
        }
        ...
        if tokenizer.code_point == Some('(') {
            ...
            tokenizer.add_token(TokenType::Regexp, regexp_pos, regexp_start, regexp_len);
            continue;
        }
    ...
    tokenizer.add_token_with_default_len(TokenType::End, tokenizer.index, tokenizer.index);
