...
pub fn process_construct_pattern_input(
    ...
    let init = match input {
        StringOrInit::String(pattern) => {
            let base_url = base_url.map(Url::parse).transpose().map_err(Error::Url)?;
            crate::UrlPatternInit::parse_constructor_string::<regex::Regex>(&pattern, base_url)?
        }
...
impl From<crate::matcher::InnerMatcher<EcmaRegexp>> for InnerMatcher {
    fn from(inner: crate::matcher::InnerMatcher<EcmaRegexp>) -> Self {
        match inner {
            crate::matcher::InnerMatcher::Literal { literal } => Self::Literal { literal },
...
pub fn parse_pattern(
    ...
    let pattern = crate::UrlPattern::<EcmaRegexp>::parse_internal(init, false, options)?;
...
pub fn parse_match_input(input: crate::UrlPatternMatchInput) -> Option<MatchInput> {
