...
fn assign_error_code(
    output: &mut BrotliDecoderErrorCode,
...
macro_rules! SaveErrorCode {
    ($state: expr, $e:expr) => {
        match assign_error_code(&mut $state.error_code, $e) {
            BrotliDecoderErrorCode::BROTLI_DECODER_SUCCESS => BrotliResult::ResultSuccess,
            BrotliDecoderErrorCode::BROTLI_DECODER_NEEDS_MORE_INPUT => BrotliResult::NeedsMoreInput,
...
pub fn BROTLI_DISTANCE_ALPHABET_SIZE(NPOSTFIX: u32, NDIRECT: u32, MAXNBITS: u32) -> u32 {
    NUM_DISTANCE_SHORT_CODES + (NDIRECT) + ((MAXNBITS) << ((NPOSTFIX) + 1))
}
...
fn DecodeSymbol(bits: u32, table: &[HuffmanCode], br: &mut bit_reader::BrotliBitReader) -> u32 {
    ...
    if table_element.bits > HUFFMAN_TABLE_BITS as u8 {
        ...
        table_element = fast!(
            (table)[(table_index
                + ((bits >> HUFFMAN_TABLE_BITS) & bit_reader::BitMask(nbits as u32)))
                as usize]
        );
    }
...
fn SafeDecodeSymbol(
    ...
    let table_sub_element =
        fast!((table)[table_index + table_element.value as usize + val as usize]);
...
>(
    ...
    for symbols_list_item in fast!((s.symbols_lists_array)[0; num_symbols as usize]).iter() {
        for other_item in
            fast!((s.symbols_lists_array)[i as usize + 1 ; num_symbols as usize+ 1]).iter()
        {
...
fn ProcessSingleCodeLength(
    ...
    *repeat = 0;
    if (code_len != 0) {
        ...
        fast_mut!(
            (symbol_lists)[(symbol_list_index_offset as i32
                + fast_inner!((next_symbol)[code_len as usize]))
                as usize]
        ) = (*symbol) as u16;
        ...
        *space = space.wrapping_sub(32768 >> code_len);
        ...
        BROTLI_LOG!(
            "[ReadHuffmanCode] code_length[{:}]={:} histo[]={:}\n",
            *symbol,
            code_len,
            code_length_histo[code_len as usize]
        );
    }
    (*symbol) += 1;
}
...
fn ProcessRepeatedCodeLength(
    ...
    *repeat += repeat_delta + 3;
    ...
    BROTLI_LOG!(
        "[ReadHuffmanCode] code_length[{:}..{:}] = {:}\n",
        *symbol,
        *symbol + repeat_delta - 1,
        *repeat_code_len
    );
    if (*repeat_code_len != 0) {
        ...
        loop {
            fast_mut!((symbol_lists)[(symbol_lists_index as i32 + next) as usize]) =
                (*symbol) as u16;
            next = (*symbol) as i32;
            (*symbol) += 1;
            if *symbol == last {
...
>(
    ...
    while (symbol < alphabet_size && space > 0) {
        let mut p_index = 0;
        let code_len: u32;
        if (!bit_reader::BrotliCheckInputAmount(
            &s.br,
        ...
        if (code_len < kCodeLengthRepeatCode) {
            ...
            let repeat_delta: u32 =
                bit_reader::BrotliGetBitsUnmasked(&s.br) as u32 & bit_reader::BitMask(extra_bits);
...
>(
    ...
    while (s.symbol < alphabet_size && s.space > 0) {
        ...
        p_index +=
            bits & bit_reader::BitMask(huffman::BROTLI_HUFFMAN_MAX_CODE_LENGTH_CODE_LENGTH as u32);
        let p = fast!((s.table)[p_index as usize]);
    ...
    loop {
        match s.substate_huffman {
            BrotliRunningHuffmanState::BROTLI_STATE_HUFFMAN_NONE => {
                ...
                if (s.sub_loop_counter != 1) {
                    ...
                    let max_code_len_len =
                        huffman::BROTLI_HUFFMAN_MAX_CODE_LENGTH_CODE_LENGTH as usize + 1;
                    for code_length_histo in
                        fast_mut!((s.code_length_histo)[0;max_code_len_len]).iter_mut()
                    {
            ...
            BrotliRunningHuffmanState::BROTLI_STATE_HUFFMAN_LENGTH_SYMBOLS => {
                ...
                table_size = huffman::BrotliBuildHuffmanTable(
                    fast_mut!((table)[offset;]),
                    HUFFMAN_TABLE_BITS as i32,
                    &s.symbols_lists_array[..],
...
fn SafeReadBlockLengthFromIndex<AllocHC: alloc::Allocator<HuffmanCode>>(
...
//
fn InverseMoveToFrontTransform(
    v: &mut [u8],
    v_len: u32,
    mtf: &mut [u8; 256],
    ...
    for (i, item) in fast_mut!((mtf)[0;(upper_bound as usize + 1usize)])
        .iter_mut()
        .enumerate()
    {
        *item = i as u8;
...
>(
    ...
    if group_index == 0 {
        hcodes = mem::replace(
            &mut s.literal_hgroup.codes,
            AllocHC::AllocatedMemory::default(),
        );
        htrees = mem::replace(
            &mut s.literal_hgroup.htrees,
        ...
        hcodes = mem::replace(
            &mut s.insert_copy_hgroup.codes,
            AllocHC::AllocatedMemory::default(),
        );
        htrees = mem::replace(
            &mut s.insert_copy_hgroup.htrees,
            AllocU32::AllocatedMemory::default(),
        );
        ...
        hcodes = mem::replace(
            &mut s.distance_hgroup.codes,
            AllocHC::AllocatedMemory::default(),
        );
        htrees = mem::replace(
            &mut s.distance_hgroup.htrees,
            AllocU32::AllocatedMemory::default(),
        );
    ...
    loop {
        match s.substate_context_map {
            BrotliRunningContextMapState::BROTLI_STATE_CONTEXT_MAP_NONE => {
                result =
                    DecodeVarLenUint8(&mut s.substate_decode_uint8, &mut s.br, num_htrees, input);
                match result {
                    BrotliDecoderErrorCode::BROTLI_DECODER_SUCCESS => {}
                    _ => return result,
                }
            ...
            BrotliRunningContextMapState::BROTLI_STATE_CONTEXT_MAP_HUFFMAN => {
                let mut local_context_map_table = mem::replace(
                    &mut s.context_map_table,
                    AllocHC::AllocatedMemory::default(),
                );
...
>(
    ...
    let (mut num_htrees, mut context_map_arg) = if is_dist_context_map {
        (
            s.num_dist_htrees,
            mem::replace(&mut s.dist_context_map, AllocU8::AllocatedMemory::default()),
        )
    } else {
        (
            s.num_literal_htrees,
            mem::replace(&mut s.context_map, AllocU8::AllocatedMemory::default()),
        )
    };

    let retval = DecodeContextMapInner(
        context_map_size as u32,
        &mut num_htrees,
        &mut context_map_arg,
        &mut s,
        input,
...
// Reads 3..54 bits.
fn DecodeBlockTypeAndLength<AllocHC: alloc::Allocator<HuffmanCode>>(
    safe: bool,
    s: &mut BlockTypeAndLengthState<AllocHC>,
    br: &mut bit_reader::BrotliBitReader,
    tree_type: i32,
...
fn DetectTrivialLiteralBlockTypes<
    AllocU8: alloc::Allocator<u8>,
    AllocU32: alloc::Allocator<u32>,
    AllocHC: alloc::Allocator<HuffmanCode>,
>(
...
>(
    ...
    s.dist_htree_index = fast_slice!(
        (s.dist_context_map)[s.dist_context_map_slice_index + s.distance_context as usize]
    );
    true
}
...
>(
    ...
    if s.should_wrap_ringbuffer {
        let (ring_buffer_start, ring_buffer_end) = s
            .ringbuffer
            .slice_mut()
            .split_at_mut(s.ringbuffer_size as usize);
        let pos = s.pos as usize;
        ring_buffer_start
            .split_at_mut(pos)
            .0
            .clone_from_slice(ring_buffer_end.split_at(pos).0);
        s.should_wrap_ringbuffer = false;
    }
}

fn CopyUncompressedBlockToOutput<
    AllocU8: alloc::Allocator<u8>,
    AllocU32: alloc::Allocator<u32>,
    AllocHC: alloc::Allocator<HuffmanCode>,
>(
        ...
        match s.substate_uncompressed {
            BrotliRunningUncompressedState::BROTLI_STATE_UNCOMPRESSED_NONE => {
                ...
                bit_reader::BrotliCopyBytes(
                    fast_mut!((s.ringbuffer.slice_mut())[s.pos as usize;]),
                    &mut s.br,
                    nbytes as u32,
                    input,
                );
...
>(
    ...
    let max_dict_size = s.ringbuffer_size as usize - 16;
    {
        ...
        if (is_last != 0) {
            while (s.ringbuffer_size as isize
                >= (s.custom_dict_size + s.meta_block_remaining_len as isize + 16) * 2
                && s.ringbuffer_size as isize > 32)
            {
                s.ringbuffer_size >>= 1;
            }
        }
        ...
        s.ringbuffer = s.alloc_u8.alloc_cell(
            (s.ringbuffer_size as usize
        ...
        if custom_dict.len() != 0 {
            ...
            fast_mut!((s.ringbuffer.slice_mut())[offset ; offset + s.custom_dict_size as usize])
                .clone_from_slice(custom_dict);
        }
    }
    if s.custom_dict.slice().len() != 0 {
        s.alloc_u8.free_cell(core::mem::replace(
            &mut s.custom_dict,
...
pub fn TakeDistanceFromRingBuffer<
    AllocU8: alloc::Allocator<u8>,
    AllocU32: alloc::Allocator<u32>,
    AllocHC: alloc::Allocator<HuffmanCode>,
>(
    ...
    if (s.distance_code == 0) {
        ...
        let mut v = (s.dist_rb_idx as i32
            + (kDistanceShortCodeIndexOffset as i32 >> distance_code as i32))
            as i32
            & 0x3;
...
>(
    ...
    if (!safe) {
        s.distance_code = ReadSymbol(
            fast!((distance_hgroup)[s.dist_htree_index as usize]),
            &mut s.br,
            input,
        ) as i32;
    } else {
    ...
    if (distval >= 0) {
        ...
        if (!safe && (s.distance_postfix_bits == 0)) {
            ...
            s.distance_code = (s.num_direct_distance_codes as i64
                + offset as i64
                + bit_reader::BrotliReadBits(&mut s.br, nbits, input) as i64)
                as i32;
        } else {
            ...
            s.distance_code = ((i64::from(offset) + i64::from(bits)) << s.distance_postfix_bits)
                .wrapping_add(i64::from(postfix))
                .wrapping_add(i64::from(s.num_direct_distance_codes))
                as i32;
        }
    }
    s.distance_code = s
        .distance_code
        .wrapping_sub(NUM_DISTANCE_SHORT_CODES as i32)
        .wrapping_add(1);
    fast_mut!((s.block_type_length_state.block_length)[2]) -= 1;
    true
}
...
pub fn BrotliDecoderHasMoreOutput<
    AllocU8: alloc::Allocator<u8>,
    AllocU32: alloc::Allocator<u32>,
    AllocHC: alloc::Allocator<HuffmanCode>,
>(
...
pub fn BrotliDecoderTakeOutput<
    'a,
    AllocU8: alloc::Allocator<u8>,
    AllocU32: alloc::Allocator<u32>,
    AllocHC: alloc::Allocator<HuffmanCode>,
...
pub fn BrotliDecoderIsFinished<
    AllocU8: alloc::Allocator<u8>,
    AllocU32: alloc::Allocator<u32>,
    AllocHC: alloc::Allocator<HuffmanCode>,
>(
...
pub fn BrotliDecoderGetErrorCode<
    AllocU8: alloc::Allocator<u8>,
    AllocU32: alloc::Allocator<u32>,
    AllocHC: alloc::Allocator<HuffmanCode>,
>(
...
>(
    ...
    let mut result = BrotliDecoderErrorCode::BROTLI_DECODER_SUCCESS;
    let mut saved_literal_hgroup = core::mem::replace(
        &mut s.literal_hgroup,
        HuffmanTreeGroup::<AllocU32, AllocHC>::default(),
    );
    let mut saved_distance_hgroup = core::mem::replace(
        &mut s.distance_hgroup,
        HuffmanTreeGroup::<AllocU32, AllocHC>::default(),
    );
    let mut saved_insert_copy_hgroup = core::mem::replace(
        ...
        loop {
            match s.state {
                ...
                BrotliRunningState::BROTLI_STATE_COMMAND_INNER => {
                    ...
                    if (s.trivial_literal_context != 0) {
                        ...
                        let mut literal_htree =
                            &fast!((literal_hgroup)[s.literal_htree_index as usize]);
                        ...
                        loop {
                            ...
                            if (fast!((s.block_type_length_state.block_length)[0]) == 0) {
                                ...
                                literal_htree =
                                    fast_ref!((literal_hgroup)[s.literal_htree_index as usize]);
                                PreloadSymbol(
                                    safe,
                                    literal_htree,
                                    &mut s.br,
                        ...
                        let mut p1 =
                            fast_slice!((s.ringbuffer)[((pos - 1) & s.ringbuffer_mask) as usize]);
                        let mut p2 =
                            fast_slice!((s.ringbuffer)[((pos - 2) & s.ringbuffer_mask) as usize]);
                        ...
                        loop {
                            ...
                            let context =
                                s.context_lookup[p1 as usize] | s.context_lookup[p2 as usize | 256];
                            BROTLI_LOG_UINT!(p1);
                            BROTLI_LOG_UINT!(p2);
                            ...
                            {
                                let i = fast_slice!(
                                    (s.context_map)[s.context_map_slice_index + context as usize]
                                );
                            ...
                            BROTLI_LOG_UINT!(
                                s.context_map.slice()
                                    [s.context_map_slice_index as usize + context as usize]
                            );
                ...
                BrotliRunningState::BROTLI_STATE_COMMAND_POST_DECODE_LITERALS => {
                    ...
                    // postReadDistance:
                    BROTLI_LOG!(
                        "[ProcessCommandsInternal] pos = %d distance = %d\n",
                        pos,
                        s.distance_code
                    );
                    ...
                    if (s.distance_code > s.max_distance) {
                        ...
                        {
                            let mut offset =
                                fast!((kBrotliDictionaryOffsetsByLength)[i as usize]) as i32;
                ...
                BrotliRunningState::BROTLI_STATE_COMMAND_POST_WRAP_COPY => {
                    ...
                    while i > 0 {
                        i -= 1;
                        fast_slice_mut!((s.ringbuffer)[pos as usize]) = fast_slice!(
                            (s.ringbuffer)[((pos - s.distance_code) & s.ringbuffer_mask) as usize]
                        );
                        pos += 1;
                        wrap_guard -= 1;
                        if (wrap_guard == 0) {
        ...
        match result {
            ...
            _ => {
                match result {
                    BrotliDecoderErrorCode::BROTLI_DECODER_NEEDS_MORE_INPUT => {
                        if s.ringbuffer.slice().len() != 0 {
                            let (intermediate_result, _) = WriteRingBuffer(
                                available_out,
                                Some(&mut output),
                                &mut output_offset,
                                &mut total_out,
                        ...
                        if s.buffer_length != 0 {
                            ...
                            if s.br.avail_in == 0 {
                                ...
                                assert_eq!(
                                    fast!((saved_buffer)[s.buffer_length as usize]),
                                    new_byte
                                );
                                s.buffer_length += 1;
                            ...
                            *available_in = s.br.avail_in as usize;
                            while *available_in != 0 {
                                fast_mut!((s.buffer)[s.buffer_length as usize]) =
                                    fast!((xinput)[*input_offset]);
                                s.buffer_length += 1;
                                (*input_offset) += 1;
                                (*available_in) -= 1;
                            }
        ...
        loop {
            ...
            match s.state {
                ...
                BrotliRunningState::BROTLI_STATE_LARGE_WINDOW_BITS => {
                    if (!bit_reader::BrotliSafeReadBits(
                        &mut s.br,
                        6,
                        &mut s.window_bits,
                        local_input,
                ...
                BrotliRunningState::BROTLI_STATE_INITIALIZE => {
                    ...
                    s.max_backward_distance_minus_custom_dict_size =
                        (s.max_backward_distance as isize - s.custom_dict_size) as i32;

                    // (formerly) Allocate memory for both block_type_trees and block_len_trees.
                    s.block_type_length_state.block_type_trees = s
                        .alloc_hc
                    ...
                    s.block_type_length_state.block_len_trees = s
                        .alloc_hc
                        .alloc_cell(3 * huffman::BROTLI_HUFFMAN_MAX_TABLE_SIZE as usize);
                ...
                BrotliRunningState::BROTLI_STATE_HUFFMAN_CODE_0 => {
                    ...
                    fast_mut!(
                        (s.block_type_length_state.num_block_types)[s.loop_counter as usize]
                    ) += 1;
                    BROTLI_LOG_UINT!(
                        s.block_type_length_state.num_block_types[s.loop_counter as usize]
                    );
                    if fast!((s.block_type_length_state.num_block_types)[s.loop_counter as usize])
                        < 2
                    {
                        s.loop_counter += 1;
                        break;
                ...
                BrotliRunningState::BROTLI_STATE_HUFFMAN_CODE_1 => {
                    let tree_offset =
                        s.loop_counter as u32 * huffman::BROTLI_HUFFMAN_MAX_TABLE_SIZE as u32;
                    let mut new_huffman_table = mem::replace(
                        &mut s.block_type_length_state.block_type_trees,
                        AllocHC::AllocatedMemory::default(),
                    );
                    let loop_counter = s.loop_counter as usize;
                    let alphabet_size =
                        fast!((s.block_type_length_state.num_block_types)[loop_counter]) + 2;
                    result = ReadHuffmanCode(
                        alphabet_size,
                        alphabet_size,
                ...
                BrotliRunningState::BROTLI_STATE_HUFFMAN_CODE_2 => {
                    let tree_offset =
                        s.loop_counter * huffman::BROTLI_HUFFMAN_MAX_TABLE_SIZE as i32;
                    let mut new_huffman_table = mem::replace(
                        &mut s.block_type_length_state.block_len_trees,
                        AllocHC::AllocatedMemory::default(),
                    );
                    result = ReadHuffmanCode(
                ...
                BrotliRunningState::BROTLI_STATE_HUFFMAN_CODE_3 => {
                    ...
                    ind_ret = SafeReadBlockLengthIndex(
                    ...
                    if !SafeReadBlockLengthFromIndex(
                    ...
                    fast_mut!((s.block_type_length_state.block_length)[s.loop_counter as usize]) =
                        block_length_out;
                    BROTLI_LOG_UINT!(
                        s.block_type_length_state.block_length[s.loop_counter as usize]
                    );
                    s.loop_counter += 1;
                    s.state = BrotliRunningState::BROTLI_STATE_HUFFMAN_CODE_0;
                    break;
                ...
                BrotliRunningState::BROTLI_STATE_CONTEXT_MAP_1 => {
                    result = DecodeContextMap(
                        (fast!((s.block_type_length_state.num_block_types)[0]) as usize)
                ...
                BrotliRunningState::BROTLI_STATE_CONTEXT_MAP_2 => {
                    let num_direct_codes = s.num_direct_distance_codes - NUM_DISTANCE_SHORT_CODES;
                    ...
                    let max_distance_symbol = if s.large_window {
                        BrotliMaxDistanceSymbol(num_direct_codes, s.distance_postfix_bits)
                    } else {
                        num_distance_codes
                    };
                    result = DecodeContextMap(
                        (fast!((s.block_type_length_state.num_block_types)[2]) as usize)
