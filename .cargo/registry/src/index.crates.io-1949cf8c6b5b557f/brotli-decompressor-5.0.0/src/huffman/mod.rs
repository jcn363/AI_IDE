...
impl<AllocU32: alloc::Allocator<u32>, AllocHC: alloc::Allocator<HuffmanCode>>
    HuffmanTreeGroup<AllocU32, AllocHC>
{
    pub fn init(
        self: &mut Self,
        mut alloc_u32: &mut AllocU32,
        mut alloc_hc: &mut AllocHC,
        alphabet_size: u16,
        max_symbol: u16,
        ntrees: u16,
    ) {
        ...
        let _ = core::mem::replace(&mut self.htrees, alloc_u32.alloc_cell(nt));
    ...
    pub fn reset(self: &mut Self, alloc_u32: &mut AllocU32, alloc_hc: &mut AllocHC) {
        alloc_u32.free_cell(core::mem::replace(
            &mut self.htrees,
            AllocU32::AllocatedMemory::default(),
        ));
        alloc_hc.free_cell(core::mem::replace(
            &mut self.codes,
...
impl<AllocU32: alloc::Allocator<u32>, AllocHC: alloc::Allocator<HuffmanCode>> Default
    for HuffmanTreeGroup<AllocU32, AllocHC>
{
    fn default() -> Self {
        HuffmanTreeGroup::<AllocU32, AllocHC> {
...
pub fn BrotliBuildCodeLengthsHuffmanTable(
    ...
    assert!(
        BROTLI_HUFFMAN_MAX_CODE_LENGTH_CODE_LENGTH as usize <= BROTLI_REVERSE_BITS_MAX as usize
    );
...
pub fn BrotliBuildHuffmanTable(
    ...
    assert!(
        BROTLI_HUFFMAN_MAX_CODE_LENGTH as isize - root_bits as isize
    ...
    loop {
        ...
        while bits_count != 0 {
            symbol =
                fast!((symbol_lists)[(symbol_lists_offset as isize + symbol as isize) as usize])
                    as i32;
    ...
    while total_size != table_size {
        for index in 0..table_size {
            ...
            fast_mut!(
                (root_table)[table_free_offset as usize + table_size as usize + index as usize]
...
pub fn BrotliBuildSimpleHuffmanTable(
    table: &mut [HuffmanCode],
    root_bits: i32,
    val: &[u16],
    num_symbols: u32,
