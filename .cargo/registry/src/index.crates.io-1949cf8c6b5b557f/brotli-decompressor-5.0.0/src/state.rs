...
> {
    ...
    pub symbols_lists_array:
        [u16; BROTLI_HUFFMAN_MAX_CODE_LENGTH + 1 + BROTLI_HUFFMAN_MAX_CODE_LENGTHS_SIZE],
...
macro_rules! make_brotli_state {
    ($alloc_u8 : expr, $alloc_u32 : expr, $alloc_hc : expr, $custom_dict : expr, $custom_dict_len: expr) => {
        BrotliState::<AllocU8, AllocU32, AllocHC> {
            ...
            symbols_lists_array: [0; BROTLI_HUFFMAN_MAX_CODE_LENGTH
                + 1
                + BROTLI_HUFFMAN_MAX_CODE_LENGTHS_SIZE],
            next_symbol: [0; 32],
    ...
    > BrotliState<AllocU8, AllocU32, AllocHC>
{
    pub fn new(alloc_u8: AllocU8, alloc_u32: AllocU32, alloc_hc: AllocHC) -> Self {
        let mut retval = make_brotli_state!(
            alloc_u8,
            alloc_u32,
            alloc_hc,
        ...
        retval.context_map_table = retval
            .alloc_hc
            .alloc_cell(BROTLI_HUFFMAN_MAX_TABLE_SIZE as usize);
    ...
    pub fn new_with_custom_dictionary(
        ...
        let mut retval =
            make_brotli_state!(alloc_u8, alloc_u32, alloc_hc, custom_dict, custom_dict_len);
        retval.context_map_table = retval
            .alloc_hc
            .alloc_cell(BROTLI_HUFFMAN_MAX_TABLE_SIZE as usize);
        retval.large_window = true;
    ...
    pub fn BrotliStateMetablockBegin(self: &mut Self) {
        ...
        self.alloc_u8.free_cell(core::mem::replace(
            &mut self.context_map,
            AllocU8::AllocatedMemory::default(),
        ));
        self.alloc_u8.free_cell(core::mem::replace(
            &mut self.context_modes,
            AllocU8::AllocatedMemory::default(),
        ));
        self.alloc_u8.free_cell(core::mem::replace(
        ...
        self.literal_hgroup
            .reset(&mut self.alloc_u32, &mut self.alloc_hc);
        self.insert_copy_hgroup
            .reset(&mut self.alloc_u32, &mut self.alloc_hc);
        self.distance_hgroup
            .reset(&mut self.alloc_u32, &mut self.alloc_hc);
    }
    ...
    fn BrotliStateCleanup(self: &mut Self) {
        ...
        self.alloc_hc.free_cell(core::mem::replace(
            &mut self.block_type_length_state.block_type_trees,
            AllocHC::AllocatedMemory::default(),
        ));
        self.alloc_hc.free_cell(core::mem::replace(
            &mut self.block_type_length_state.block_len_trees,
            AllocHC::AllocatedMemory::default(),
        ));
        self.alloc_hc.free_cell(core::mem::replace(
            &mut self.context_map_table,
            AllocHC::AllocatedMemory::default(),
        ));
        self.alloc_u8.free_cell(core::mem::replace(
            &mut self.custom_dict,
            AllocU8::AllocatedMemory::default(),
        ));
    ...
    pub fn BrotliStateIsStreamStart(self: &Self) -> bool {
        match self.state {
            BrotliRunningState::BROTLI_STATE_UNINITED => BrotliGetAvailableBits(&self.br) == 0,
            _ => false,
        }
    }
    ...
    pub fn BrotliHuffmanTreeGroupInit(
        ...
        match group {
            WhichTreeGroup::LITERAL => self.literal_hgroup.init(
                &mut self.alloc_u32,
                &mut self.alloc_hc,
                alphabet_size,
                max_symbol,
            ...
            WhichTreeGroup::INSERT_COPY => self.insert_copy_hgroup.init(
                &mut self.alloc_u32,
                &mut self.alloc_hc,
                alphabet_size,
                max_symbol,
            ...
            WhichTreeGroup::DISTANCE => self.distance_hgroup.init(
                &mut self.alloc_u32,
                &mut self.alloc_hc,
                alphabet_size,
                max_symbol,
    ...
    pub fn BrotliHuffmanTreeGroupRelease(self: &mut Self, group: WhichTreeGroup) {
        match group {
            WhichTreeGroup::LITERAL => self
                .literal_hgroup
                .reset(&mut self.alloc_u32, &mut self.alloc_hc),
            WhichTreeGroup::INSERT_COPY => self
                .insert_copy_hgroup
                .reset(&mut self.alloc_u32, &mut self.alloc_hc),
            WhichTreeGroup::DISTANCE => self
                .distance_hgroup
                .reset(&mut self.alloc_u32, &mut self.alloc_hc),
        }
    }
...
impl<
        ...
        AllocU32: alloc::Allocator<u32>,
        AllocHC: alloc::Allocator<HuffmanCode>,
    > Drop for BrotliState<AllocU8, AllocU32, AllocHC>
{
    fn drop(&mut self) {
