...
pub mod interface;
use self::{
    alloc_util::SubclassableAllocator,
    interface::{
        brotli_alloc_func,
        brotli_free_func,
        c_void,
...
pub struct BrotliDecoderState {
    ...
    pub decompressor:
        ::BrotliState<SubclassableAllocator, SubclassableAllocator, SubclassableAllocator>,
}

#[cfg(not(feature = "std"))]
fn brotli_new_decompressor_without_custom_alloc(
    _to_box: BrotliDecoderState,
...
#[cfg(feature = "std")]
fn brotli_new_decompressor_without_custom_alloc(
    to_box: BrotliDecoderState,
) -> *mut BrotliDecoderState {
    alloc_util::Box::<BrotliDecoderState>::into_raw(alloc_util::Box::<BrotliDecoderState>::new(
        to_box,
    ))
}
...
pub unsafe extern "C" fn BrotliDecoderCreateInstance(
    ...
    match catch_panic_state(|| {
        ...
        let custom_dictionary =
            <SubclassableAllocator as Allocator<u8>>::AllocatedMemory::default();
        ...
        if let Some(alloc) = alloc_func {
            ...
            let brotli_decoder_state_ptr =
                core::mem::transmute::<*mut c_void, *mut BrotliDecoderState>(ptr);
...
pub unsafe extern "C" fn BrotliDecoderDecompress(
    ...
    let res = BrotliDecoderDecompressWithReturnInfo(
        encoded_size,
        encoded_buffer,
        *decoded_size,
        decoded_buffer,
...
fn catch_panic<F: FnOnce() -> BrotliDecoderResult + panic::UnwindSafe>(
    f: F,
...
#[cfg(all(feature = "std", not(feature = "pass-through-ffi-panics")))]
fn catch_panic_state<F: FnOnce() -> *mut BrotliDecoderState + panic::UnwindSafe>(
    f: F,
    ...
    if let Some(st) = err.downcast_ref::<&str>() {
        if !state_ptr.is_null() {
            ...
            str_cpy
                .split_at_mut(xlen)
                .0
                .clone_from_slice(src.split_at(xlen).0);
...
fn catch_panic<F: FnOnce() -> BrotliDecoderResult>(
    f: F,
) -> Result<BrotliDecoderResult, BrotliAdditionalErrorData> {
    Ok(f())
}
...
fn catch_panic_state<F: FnOnce() -> *mut BrotliDecoderState>(
    f: F,
) -> Result<*mut BrotliDecoderState, BrotliAdditionalErrorData> {
    Ok(f())
}
    ...
    match catch_panic(move || {
        ...
        *output_buf_ptr = (*output_buf_ptr).offset(output_offset as isize);
        ...
        Err(mut readable_err) => {
            // if we panic (completely unexpected) then we should report it back to C and print
...
#[no_mangle]
pub unsafe extern "C" fn BrotliDecoderMallocUsize(
    state_ptr: *mut BrotliDecoderState,
    size: usize,
) -> *mut usize {
    if let Some(alloc_fn) = (*state_ptr).custom_allocator.alloc_func {
...
#[no_mangle]
pub unsafe extern "C" fn BrotliDecoderFreeUsize(
    state_ptr: *mut BrotliDecoderState,
    data: *mut usize,
    size: usize,
) {
...
pub unsafe extern "C" fn BrotliDecoderGetErrorString(
    ...
    BrotliDecoderErrorString(super::decode::BrotliDecoderGetErrorCode(
        &(*state_ptr).decompressor,
    ))
}
#[no_mangle]
pub extern "C" fn BrotliDecoderErrorString(c: BrotliDecoderErrorCode) -> *const u8 {
