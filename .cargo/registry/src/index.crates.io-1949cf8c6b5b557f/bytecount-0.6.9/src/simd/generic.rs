...
pub fn chunk_count(haystack: &[u8], needle: u8) -> usize {
    ...
    unsafe {
        ...
        while haystack.len() >= offset + 64 * 255 {
            ...
            for _ in 0..255 {
                counts -= u8x64_from_offset(haystack, offset)
                    .simd_eq(needles_x64)
                    .to_int()
                    .cast();
                offset += 64;
        ...
        for i in 0..(haystack.len() - offset) / 32 {
            counts -= u8x32_from_offset(haystack, offset + i * 32)
                .simd_eq(needles_x32)
                .to_int()
                .cast();
        }
        ...
        if haystack.len() % 32 != 0 {
            counts -= u8x32_from_offset(haystack, haystack.len() - 32)
                .simd_eq(needles_x32)
                .to_int()
                .cast()
...
fn is_leading_utf8_byte_x64(u8s: u8x64) -> u8x64 {
    (u8s & u8x64::splat(0b1100_0000))
        .simd_ne(u8x64::splat(0b1000_0000))
        .to_int()
        .cast()
}

fn is_leading_utf8_byte_x32(u8s: u8x32) -> u8x32 {
    (u8s & u8x32::splat(0b1100_0000))
        .simd_ne(u8x32::splat(0b1000_0000))
        .to_int()
        .cast()
}
