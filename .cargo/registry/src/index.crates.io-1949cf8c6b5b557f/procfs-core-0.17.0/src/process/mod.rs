...
impl MMapPath {
    pub fn from(path: &str) -> ProcResult<MMapPath> {
        Ok(match path.trim() {
            ...
            x if x.starts_with("/SYSV") => {
                MMapPath::Vsys(u32::from_str_radix(&x[5..13], 16)? as i32)
            } // 32bits signed hex. /SYSVaabbccdd (deleted)
...
impl crate::FromBufRead for MemoryMaps {
    ...
    fn from_buf_read<R: BufRead>(reader: R) -> ProcResult<Self> {
        ...
        let mut line_iter = reader
            .lines()
            .map(|r| r.map_err(|_| ProcError::Incomplete(None)));
                        ...
                        if line.starts_with("VmFlags") {
                            ...
                            if let (Some(k), Some(v)) = (key, value) {
                                ...
                                let v = v.parse::<u64>().map_err(|_| {
                                    ProcError::Other(
                                        "Value in `Key: Value` pair was not actually a number"
                                            .into(),
                                    )
                                })?;
