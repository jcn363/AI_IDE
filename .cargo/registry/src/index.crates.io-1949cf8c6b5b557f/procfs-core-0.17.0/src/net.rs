    ...
    fn from_buf_read<R: BufRead>(r: R, system_info: &crate::SystemInfo) -> ProcResult<Self> {
        ...
        for line in r.lines().skip(1) {
            ...
            vec.push(TcpNetEntry {
                local_address: parse_addressport_str(
                    local_address,
                    system_info.is_little_endian(),
                )?,
...
impl super::FromBufRead for RouteEntries {
    fn from_buf_read<R: BufRead>(r: R) -> ProcResult<Self> {
        ...
        for line in r.lines().skip(1) {
            ...
            let destination = from_str!(u32, expect!(line.next()), 16)
                .to_ne_bytes()
                .into();
            let gateway = from_str!(u32, expect!(line.next()), 16)
                .to_ne_bytes()
                .into();
            ...
            let mask = from_str!(u32, expect!(line.next()), 16)
                .to_ne_bytes()
                .into();
