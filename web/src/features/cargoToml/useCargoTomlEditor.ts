...
import { CargoDependency, CargoManifest, DependencySection, parseCargoToml, stringifyCargoToml } from '../../types/cargo';
...
export function useCargoTomlEditor(initialToml: string) {
  ...
  const parseToml = useCallback(async (toml: string) => {
    try {
      ...
      const licenseCompatibility: LicenseCompatibility = {
        ...
        conflicts: licenseSummary.incompatibleCount > 0 ? [{ package: 'Unknown', license: 'Incompatible' }] : [],
  ...
  // Update TOML content
  const updateToml = useCallback((newToml: string) => {
    setState(prev => ({
      ...prev,
  ...
  const updateDependency = useCallback(async (updates: Array<{ name: string; version: string }>) => {
    if (!state.manifest) return;
    ...
    updates.forEach(({ name, version }) => {
      ...
      if (newManifest.dependencies?.[name]) {
        ...
        } else if (newManifest.dependencies[name] && typeof newManifest.dependencies[name] === 'object') {
      ...
      if (newManifest['dev-dependencies']?.[name]) {
        ...
        } else if (newManifest['dev-dependencies'][name] && typeof newManifest['dev-dependencies'][name] === 'object') {
      ...
      if (newManifest['build-dependencies']?.[name]) {
        ...
        } else if (newManifest['build-dependencies'][name] && typeof newManifest['build-dependencies'][name] === 'object') {
  ...
  const addDependency = useCallback((name: string, version: string, isDev = false) => {
    if (!state.manifest) return;
