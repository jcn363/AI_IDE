...
export const useAIAssistant = () => {
  ...
  useEffect(() => {
    ...
    const initializeServices = async () => {
      try {
        ...
        if (isMounted) {
          const errorMessage = error instanceof Error ? `Failed to initialize AI services: ${error.message}` : 'Failed to initialize AI services';
  ...
  const analyzeCurrentFile = useCallback(async (
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? `Failed to analyze file ${filePath}: ${error.message}` : `Failed to analyze file ${filePath}`;
      setGenerationError(errorMessage);
  ...
  const analyzeWorkspace = useCallback(async (
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? `Failed to analyze workspace ${workspacePath}: ${error.message}` : `Failed to analyze workspace ${workspacePath}`;
      setGenerationError(errorMessage);
      throw error;
    } finally {
      setAnalysisState(prev => ({ ...prev, isAnalyzing: false }));
  ...
  const analyzeCodeStyle = useCallback(async (
    code: string,
    filePath: string,
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to analyze code style';
      setGenerationError(errorMessage);
      return [];
    }
  }, []);
  ...
  const analyzeArchitecture = useCallback(async (
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? `Failed to analyze architecture: ${error.message}` : 'Failed to analyze architecture';
      setGenerationError(errorMessage);
      return [];
    } finally {
      setAnalysisState(prev => ({ ...prev, isAnalyzing: false }));
  ...
  const getEnhancedPerformanceHints = useCallback(async (
    code: string,
    filePath: string,
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get performance hints';
      setGenerationError(errorMessage);
      return [];
    }
  }, []);
  ...
  const recordSuccessfulFix = useCallback(async (
  ...
  const getCompilerDiagnostics = useCallback(async (
    filePath?: string,
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get compiler diagnostics';
      setGenerationError(errorMessage);
      return [];
    }
  }, []);
  ...
  const explainErrorCode = useCallback(async (
    errorCode: string
  ): Promise<ErrorCodeExplanation | null> => {
    try {
      setGenerationError(null);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to explain error code';
      setGenerationError(errorMessage);
      return null;
    }
  }, []);
  ...
  const resolveErrorWithLearning = useCallback(async (
    errorMessage: string,
    filePath: string,
    context: string
...
const applyFixWithLearning = useCallback(async (
    suggestionId: string,
    changes: CodeChange[],
    createBackup = true,
    ...
    try {
      ...
      const affectedFiles = [...new Set(changes.map(change => (change.file_path ?? (change as any).filePath)))].filter(Boolean);
  ...
  const runEnhancedAnalysis = useCallback(async (
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? `Failed to run enhanced analysis for ${filePath}: ${error.message}` : `Failed to run enhanced analysis for ${filePath}`;
      setGenerationError(errorMessage);
      throw error;
    } finally {
      setAnalysisState(prev => ({ ...prev, isAnalyzing: false }));
  ...
  const getPerformanceSuggestions = useCallback(async (
    options: PerformanceSuggestionsOptions = {}
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get performance suggestions';
      setGenerationError(errorMessage);
      return [];
    }
  }, [aiServiceRef]);
  ...
  const runCodeQualityCheck = useCallback(async (
    targetPath: string,
    options: CodeQualityCheckOptions = {}
  ) => {
    try {
      setGenerationError(null);
      setAnalysisState(prev => ({ ...prev, isAnalyzing: true }));
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to run code quality check';
      setGenerationError(errorMessage);
      setAnalysisState(prev => ({ ...prev, isAnalyzing: false }));
      throw error;
    }
  ...
  const applySuggestion = useCallback(async (
    ...
    try {
      ...
      const result = await aiServiceRef.current.applySuggestion(suggestionId, changes, createBackup);
  ...
  const generateCode = useCallback(async (options: CodeGenerationOptions) => {
    try {
      ...
      const analysisResult = options.filePath ? 
        analysisState.analysisResults.get(options.filePath) : undefined;
  ...
  const updateConfiguration = useCallback(async (newConfig: Partial<AIAnalysisConfigExtended>) => {
    try {
      if (!aiServiceRef.current) {
        throw new Error('AI service not initialized');
      }
  ...
  const updateEnhancedConfiguration = useCallback(async (
    newConfig: Partial<AnalysisConfiguration>
  ): Promise<void> => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to update enhanced configuration';
      setGenerationError(errorMessage);
      throw error;
    }
  }, [codeAnalyzerRef]);
  ...
  const cancelAnalysis = useCallback(async (filePath?: string) => {
    try {
      if (filePath) {
      ...
      log.error('Failed to cancel analysis', error instanceof Error ? error : undefined, { filePath });
    }
  ...
  const getLearningStats = useCallback(() => {
    return {
      ...
      averageConfidence: analysisState.learnedPatterns.length > 0 
        ? analysisState.learnedPatterns.reduce((sum, pattern) => sum + pattern.confidence, 0) / analysisState.learnedPatterns.length
        : 0,
      successfulFixesCount: analysisState.learnedPatterns.reduce((sum, pattern) => sum + pattern.successCount, 0),
      failedFixesCount: analysisState.learnedPatterns.reduce((sum, pattern) => sum + pattern.failureCount, 0),
  ...
  const generateTests = useCallback(async (code: string, filePath: string) => {
  ...
  const generateDocumentation = useCallback(async (code: string, filePath: string) => {
    const options: CodeGenerationOptions = {
      context: code,
      cursorPosition: { line: 0, character: 0 },
      fileContent: code,
    ...
    try {
      ...
      if (aiServiceRef.current && filePath) {
        ...
        const result = await aiServiceRef.current.generateWithContext(
          {
            prompt: 'Explain this code in detail, including its purpose, functionality, and any potential issues',
            language: 'rust',
          },
          {
            currentCode: code,
      ...
      const explanation = `This code ${code.substring(0, 50)}... appears to be a function that performs some operation. ` +
  ...
  const refactorCode = useCallback(async (code: string, filePath: string) => {
    const options: CodeGenerationOptions = {
      context: code,
      cursorPosition: { line: 0, character: 0 },
      fileContent: code,
...
export type {
  AnalysisState, ArchitectureAnalysisOptions, CodeQualityCheckOptions, CompilerIntegrationOptions, LearningSystemOptions, PerformanceSuggestionsOptions, StyleAnalysisOptions
};
