...
export const useSpecificationGeneration = (): UseSpecificationGenerationReturn => {
  ...
  const generateFromSpec = useCallback(
    async (description: string, language: string = 'rust', framework?: string) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const validateSpec = useCallback(
    async (description: string): Promise<ValidationResult> => {
      try {
  ...
  const enhanceCode = useCallback(
    async (enhancements: string[]): Promise<GeneratedCode | null> => {
      if (!generatedCode) {
      ...
      try {
        ...
        const enhanced = await SpecificationService.enhanceGeneratedCode(
          generatedCode,
          enhancements
        );
  ...
  const exportCode = useCallback(
    async (basePath: string, overwrite: boolean = false) => {
      if (!generatedCode) {
      ...
      try {
        ...
        const result = await SpecificationService.exportToFilesystem(
          generatedCode,
          basePath,
          overwrite
        );
