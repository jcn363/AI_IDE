...
export const useCodeReview = (): UseCodeReviewReturn => {
  ...
  const reviewPullRequest = useCallback(
    async (targetPath: string, prUrl?: string, config?: ReviewConfig) => {
      try {
        setLoading(true);
        setError(null);
        ...
        console.log(`PR review completed:`, {
          ...
          totalComments: result.summary?.totalComments,
          recommendation:
            result.overallAssessment?.score && result.overallAssessment.score >= 0.7
              ? 'Approved'
              : 'Needs Work',
        });
      } catch (err) {
  ...
  const generateReport = useCallback(
    (format: 'markdown' | 'json' = 'markdown'): string | null => {
  ...
  const getCommentsByFile = useCallback(
    (filePath: string): ReviewComment[] => {
      ...
      return reviewResult.reviewComments.filter((comment) => comment.filePath === filePath);
    },
    [reviewResult]
  );
