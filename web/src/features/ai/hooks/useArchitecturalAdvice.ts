...
export const useArchitecturalAdvice = (config: ArchitecturalAdviceConfig = {}) => {
  ...
  const analyzeArchitecture = useCallback(async (
    ...
    try {
        ...
        primary_recommendations: [
          {
            ...
            description: 'Use dependency injection to decouple business logic from infrastructure concerns.',
            impact: 'High',
            effort: 'Medium',
            rationale: 'This will improve testability and maintainability by making dependencies explicit and easy to mock.'
          },
          {
            ...
            impact: 'High',
            effort: 'Low',
            rationale: 'Prevents invalid data from propagating through the system and improves security.'
          }
        ],
        secondary_suggestions: [
          {
            title: 'Consider implementing CQRS pattern',
            description: 'Separate command and query operations for better performance and scalability.',
            priority: 'High',
            category: 'Architecture'
          },
          {
            title: 'Add comprehensive logging and monitoring',
            description: 'Implement structured logging and performance monitoring throughout the application.',
            priority: 'Medium',
      ...
      const errorMessage = error instanceof Error ? error.message : 'Architecture analysis failed';
      setState(prev => ({
        ...prev,
        error: errorMessage,
        isLoading: false
      ...
      const mockDecisions: ArchitecturalDecision[] = [
        {
          ...
          problem: 'Need to handle user data with complex relationships and high read/write operations',
  ...
  const createDecision = useCallback(async (decision: Omit<ArchitecturalDecision, 'id' | 'made_at' | 'status'>) => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));
