...
export const ModelConfigurationPanel: React.FC<ModelConfigurationPanelProps> = ({
  ...
  const getAvailableModels = () => {
    return availableModels.filter(model =>
      !loadedModels.some(loaded => loaded.id === model.id)
    );
  };
    ...
    <div className="model-configuration-panel bg-gray-900 text-white p-6 rounded-lg">
      ...
      {selectedModel && (
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div>
            <span className="text-gray-400">Name:</span> {selectedModel.model_type} {selectedModel.model_size}
          </div>
          <div>
          ...
          <div>
            <span className="text-gray-400">Memory:</span> {selectedModel.memory_usage_mb || 'Unknown'}MB
          </div>
        </div>
          <button
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {getAvailableModels().map(model => (
            <div key={model.id} className="flex items-center justify-between p-3 bg-gray-800 rounded">
              <div className="flex-1">
                <div className="font-medium">{model.model_type} {model.model_size}</div>
                <div className="text-sm text-gray-400">
                  ~{model.memory_usage_mb || 'Unknown'}MB •
                  Supports fine-tuning: {model.supports_fine_tuning ? 'Yes' : 'No'}
                </div>
              </div>
              <button
        ...
        <div>
          ...
          <div className="space-y-2">
            {loadedModels.map(model => (
              <div key={model.id} className="flex items-center justify-between p-3 bg-green-900 rounded">
                <div className="flex-1">
                  <div className="font-medium">{model.model_type} {model.model_size}</div>
