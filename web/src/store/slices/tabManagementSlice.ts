...
splitPane: (state, action: PayloadAction<{paneId: string; direction: 'horizontal' | 'vertical'}>) => {
      ...
      // Create new panes
      state.editorPanes[newPane1Id] = createPane(newPane1Id, pane.activeFile, pane.files.map(f => f.path));
    ...
    openFileInPane: (state, action: PayloadAction<{paneId: string; filePath: string}>) => {
      ...
      if (existingFileIndex === -1) {
        ...
        const lastPinnedIndex = [...pane.files].reverse().findIndex((f: { isPinned: boolean }) => f.isPinned);
    ...
    closeFileInPane: (state, action: PayloadAction<{paneId: string; filePath: string}>) => {
      ...
      if (pane.activeFile === filePath) {
        pane.activeFile = pane.files[fileIndex]?.path || 
                         pane.files[fileIndex - 1]?.path || 
                         null;
      }
      ...
      if (pane.files.length === 0 && paneId !== state.rootPaneId) {
        ...
        const parentPane = Object.values(state.editorPanes).find(
          p => p.children?.includes(paneId)
        );
...
moveTab: (state, action: PayloadAction<{
      ...
      if (sourcePane.activeFile === movedTab.path) {
        if (sourcePane.files.length > 0) {
          sourcePane.activeFile = sourcePane.files[Math.min(sourceIndex, sourcePane.files.length - 1)].path;
        } else {
      ...
      if (file) {
        ...
        if (fileIndex !== -1) {
          ...
          if (file.isPinned) {
            ...
            const firstUnpinnedIndex = pane.files.findIndex((f: { isPinned: boolean }) => !f.isPinned);
            ...
            const lastPinnedIndex = pane.files.reduce<number>((acc, f, idx) => f.isPinned ? idx : acc, -1);
    ...
    updateDragTarget: (state, action: PayloadAction<{paneId: string | null; index: number | null}>) => {
      const { paneId, index } = action.payload;
    ...
    endDragTab: (state, action: PayloadAction<{createNewPane?: boolean; direction?: 'horizontal' | 'vertical'}>) => {
...
export const tabManagementSelectors = {
  selectActivePaneId: (state: { tabManagement: TabManagementState }) => state.tabManagement.activePaneId,
  selectEditorPanes: (state: { tabManagement: TabManagementState }) => state.tabManagement.editorPanes,
  ...
  selectDragState: (state: RootState) => state.tabManagement?.dragState || {
    isDragging: false,
    sourcePaneId: null,
    sourceIndex: null,
    targetPaneId: null,
