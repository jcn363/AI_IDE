...
export const VariablesList: React.FC<VariablesListProps> = ({
  ...
  const updateChildrenMap = useCallback(
    (varName: string, children: VariableNode[]) => {
      updateState({
        childrenMap: {
          ...childrenMap,
          [varName]: children,
  ...
  const defaultRenderVariableValue = useCallback(
  ...
  const renderVariable = useCallback(
    (variable: Variable, depth = 0): React.ReactNode => {
  ...
  const renderedVariables = useMemo(
    () =>
      variables.map((variable) => (
        <React.Fragment key={variable.name}>{renderVariable(variable)}</React.Fragment>
      )),
