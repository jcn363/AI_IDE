...
> {
  ...
  componentDidUpdate(prevProps: any) {
    if (
      prevProps.originalContent !== this.props.originalContent ||
  ...
  private computeClientSideDiff(): Omit<
    RealTimePreviewState,
    'isLoading' | 'settings' | 'zoomLevel'
  > {
  ...
  private toggleSetting(setting: keyof PreviewSettings) {
    this.setState((prevState) => ({
      settings: {
        ...prevState.settings,
        [setting]:
          typeof prevState.settings[setting] === 'boolean'
        ...
        <div className="preview-header">
          ...
          <div className="preview-controls">
            ...
            <div className="zoom-controls">
              <button
                onClick={() =>
                  this.setState((prev) => ({ zoomLevel: Math.max(0.5, prev.zoomLevel - 0.2) }))
                }
              >
                🔍-
              </button>
              ...
              <button
                onClick={() =>
                  this.setState((prev) => ({ zoomLevel: Math.min(3, prev.zoomLevel + 0.2) }))
                }
              >
                🔍+
              </button>
            ...
            {hasChanges ? (
              <div className="diff-viewer" ref={this.diffViewerRef}>
                ...
                <div
                  ...
                  style={{ fontSize: `${settings.fontSize * this.state.zoomLevel}px` }}
                >
                  {settings.showLineNumbers && this.renderLineNumbers()}
                  {settings.highlightChanges
                    ? this.renderHighlightedDiff()
    ...
    return (
      <div className="diff-statistics">
        ...
        <div className="region-navigation">
          ...
          <span>
            {this.state.currentRegion !== null ? this.state.currentRegion + 1 : 0} /{' '}
            {this.state.diffRegions.length}
          </span>
          <button
            onClick={() =>
              this.setState((prev) => ({
    ...
    for (let i = 0; i < maxLines; i++) {
      ...
      lines.push(
        <div key={i} className={lineClass}>
          <div className="original-line">{isDifferent ? originalLine : null}</div>
          <div className="modified-line">{isDifferent ? modifiedLine : originalLine}</div>
        </div>
      );
    }
  ...
  private renderHighlightedDiff() {
    ...
    for (let i = 0; i < maxLines; i++) {
      ...
      const regionIndex = diffRegions.findIndex(
        (region) => i >= region.lineStart && i <= region.lineEnd
      );
      ...
      <div className="highlighted-diff">
        {processedLines.map((line, index) => (
          <div
            ...
            onClick={() => {
              const regionIndex = diffRegions.findIndex(
                (region) => index >= region.lineStart && index <= region.lineEnd
              );
              if (regionIndex !== -1) {
    ...
    return (
      <div className="change-actions">
        <div className="action-buttons">
          <button className="btn btn-primary" onClick={() => this.applyChanges()}>
            ✅ Apply Changes
          </button>
          <button className="btn btn-secondary" onClick={() => this.discardChanges()}>
            ❌ Discard Changes
          </button>
          <button className="btn btn-outline" onClick={() => this.computeDiff()}>
            🔄 Refresh Preview
          </button>
        </div>
        ...
        <div className="action-info">
          <p>
            This preview shows how the refactoring will affect {this.state.diffRegions.length}{' '}
            regions of code. Review the changes carefully before applying.
          </p>
        </div>
      </div>
    );
