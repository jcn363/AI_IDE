...
class RefactoringTemplateManager extends React.Component<{}, TemplateManagerState> {
    ...
    private getBuiltInTemplates(): RefactoringTemplate[] {
        return [
            ...
            },
            {
                ...
                operations: ['convertToAsync', 'replaceDeprecatedApis', 'updatePatterns', 'changeSignature'],
                config: {
                    aiEnabled: true,
            ...
            },
            {
                ...
                description: 'Optimize code for better performance with memory and algorithmic improvements',
                category: 'performance',
                operations: ['optimizeMemory', 'algorithmRefinement', 'inlineCriticalPaths', 'reduceAllocations'],
                config: {
                    aiEnabled: true,
    ...
    private async saveCustomData() {
        try {
            await invoke('save_custom_templates', { templates: JSON.stringify(this.state.customTemplates) });
        ...
        this.setState(prevState => ({
            customTemplates: prevState.customTemplates.map(template =>
                template.id === templateId
                    ? { ...template, ...updates, metadata: { ...template.metadata, lastModified: Date.now() } }
                    : template
            ),
        }));
    ...
    applyTemplate = async (template: RefactoringTemplate, context?: {
        filePath?: string;
    ...
    getRecommendedTemplates = (context: {
        ...
        if (context.confidenceNeeded) {
            contextTemplates.push(...allTemplates.filter(t =>
                t.config.confidenceThreshold >= context.confidenceNeeded
            ));
        }
        ...
        const uniqueTemplates = Array.from(new Set(contextTemplates.map(t => t.id)))
            ...
            .filter(Boolean) as RefactoringTemplate[]
            .sort((a, b) => {
                const scoreA = this.getTemplateScore(a || {} as any, context);
                const scoreB = this.getTemplateScore(b || {} as any, context);
    ...
    private getTemplateScore(template: RefactoringTemplate, context: any): number {
        ...
        const difficultyBonus = template.metadata.difficulty === 'intermediate' ? 2 :
                               template.metadata.difficulty === 'beginner' ? 1 : 0;
        ...
        if (context.confidenceNeeded && template.config.confidenceThreshold >= context.confidenceNeeded) {
