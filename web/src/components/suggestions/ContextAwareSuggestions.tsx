...
import RefactoringAnalytics, {
  RefactoringEvent,
  AnalyticsData,
} from '../../utils/analytics/RefactoringAnalytics';
...
> {
  ...
  private createDeadCodeRule(): SuggestionRule {
    return {
      ...
      triggerCondition: (context) => {
        return (
          context.analytics?.userBehavior.workflowPatterns.includes('cleanup-focused') || false
        );
      },
      signature: {
        priority: 4,
  ...
  private calculateContextConfidence(
    ...
    if (
      context.userProfile.experience === 'expert' &&
      rule.signature.automationLevel === 'manual'
    ) {
      confidence += 0.1;
    }
  ...
  private findCurrentMethod(
    context: ContextualInformation
  ): { name: string; lineCount: number } | null {
        ...
        {suggestions.length > 0 ? (
          <div className="suggestions-list">
            {suggestions.map((suggestion, index) => (
              ...
              >
                <div className="suggestion-priority">#{index + 1}</div>
                ...
                <div className="suggestion-content">
                  ...
                  <div className="suggestion-automation">
                    <span
                      className={`automation-level ${suggestion.rule.signature.automationLevel}`}
                    >
                ...
                <div className="suggestion-actions">
                  ...
                  <button className="btn btn-outline btn-small">View Code</button>
                </div>
              </div>
            ))}
          </div>
