...
class RefactoringAnalytics extends React.Component {
    ...
    async analyzeUsagePatterns(): Promise<AnalyticsData> {
        ...
        const usagePatterns = Object.entries(operationsByType).map(([type, events]) => {
            ...
            const peakHour = Object.keys(usageByHour)
                .reduce((peak, hour) => usageByHour[hour].length > usageByHour[peak]?.length ? hour : peak, '0');

            const fileSizes = events.map(e => e.fileSize);
            const avgFileSize = this.median(fileSizes);
            const fileSizeTendency = avgFileSize < 500 ? 'small' :
                                   avgFileSize < 5000 ? 'medium' : 'large';
        ...
        const trending = Object.keys(recentOperations).sort((a, b) => {
    ...
    private calculateSelectionComplexity(events: RefactoringEvent[]): 'simple' | 'complex' | 'very_complex' {
        const avgSelectionLines = events.reduce((sum, e) => sum + e.selectionLines, 0) / events.length;
        const avgImpactedSymbols = events.reduce((sum, e) => sum + e.impactedSymbols, 0) / events.length;
    ...
    private calculateUserBehavior(): UserBehavior {
        ...
        if (sortedEvents.length > 1) {
            ...
            const earlyAvg = firstQuarter.reduce((sum, e) => sum + (e.confidence || 0), 0) / firstQuarter.length;
            const lateAvg = lastQuarter.reduce((sum, e) => sum + (e.confidence || 0), 0) / lastQuarter.length;
    ...
    private detectWorkflowPatterns(): string[] {
        ...
        for (let i = 0; i < events.length - 1; i++) {
            ...
            if (events[i + 1].timestamp - events[i].timestamp < 30000) { // Within 30 seconds
        ...
        if (events.filter(e => ['extractFunction', 'extractVariable'].includes(e.operationType)).length > 0.3 * events.length) {
    ...
    private generatePerformanceInsights(): string[] {
        ...
        if (userBehavior.refactoringTimeSpent > 3600000) { // 1 hour
    ...
    private generateRecommendations(): string[] {
        ...
        const avgDuration = this.events.reduce((sum, e) => sum + e.duration, 0) / Math.max(this.events.length, 1);
        if (avgDuration > 5000) {
