...
describe('Refactoring System Integration Tests', () => {
            ...
            changes: [
              {
                ...
                line_end: 15,
                col_end: 0,
                new_content: '// New function extracted',
              },
            ],
            errorMessage: null,
            warnings: [],
    ...
    it('allows selection and execution of multiple operations', async () => {
      render(
        <BatchRefactoringPanel
          currentFile={mockProps.currentFile}
          availableOperations={[]}
          onBack={jest.fn()}
        />
      );
  ...
  describe('RefactoringHistory Component', () => {
    it('loads and displays operation history', async () => {
      ...
      render(
        <RefactoringHistory
          currentFile={mockProps.currentFile}
          onHistoryEntryClicked={mockOnHistoryEntryClicked}
        />
      );
    ...
    it('handles undo/redo operations', async () => {
      ...
      render(
        <RefactoringHistory currentFile={mockProps.currentFile} onHistoryEntryClicked={jest.fn()} />
      );
  ...
  describe('Performance Testing', () => {
    ...
    it('debounces rapid updates', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        rerender(<RefactoringPanel {...mockProps} cursorPosition={{ line: i, character: 0 }} />);
      }
      ...
      await waitFor(
        () => {
          expect(
            mockInvoke.mock.calls.filter(([cmd]) => cmd === 'get_refactoring_suggestions')
          ).toHaveLength(1);
        },
        { timeout: 500 }
      );
